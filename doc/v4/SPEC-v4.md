...
为了让「下拉比元数据列表少」这件事自解释（08-07 用户实测反馈），`catalogTree` 同时返回
被隐藏的数量：顶层 `offlineCategories` + 每分类 `offlineScenes`，编辑器在两个下拉下方
提示「另有 N 个已下架未列出」；「问题场景」列表页的上级分类列在父分类未发布时
标注「（已下架）」（`listScenes().parentPublished`）——场景自身「已上架」但父分类
已下架时，它不会出现在编辑器里，这层原因必须在界面上可见。
...
| 顶栏 | 全局搜索（占位「搜索知识、场景、分类或知识 ID…」，命中下拉 ≤6 条，无命中提示引导登记未命中）；右侧「待办 N」按钮；当前用户名 + 角色 + 「切换」+ 「退出」（注销会话回登录页，08-07 用户需求） |
...
- 下拉只列**已发布**的分类/场景（§2.1）；存在被隐藏的已下架对象时，对应 select 下方提示「另有 N 个分类已下架未列出，可在元数据中心上架后选用」/「该分类下另有 N 个场景已下架未列出」，数量来自 `catalogTree` 的 `offlineCategories` / `offlineScenes`
...
| 登出 | 无（原型无登录态） | 顶栏「退出」按钮 → `POST /auth/logout` 注销会话并回登录页；接口失败也回登录页（会话已失效等同已退出）。强制改密页同样提供「退出登录」（登错账号的人不该被困在改密页，428 白名单本就放行 logout） | 会话有效期 12h，自动恢复登录态；没有登出入口用户就无法主动结束会话（08-07 用户需求） |
...
### 7.1.6 第六轮：编辑器下拉「与元数据对不上」排查（08-07-2026）

用户报「新建知识的一级分类下拉显示不完整、场景与元数据对不上」。排查结论：**下拉过滤逻辑
本身符合 §2.1（只列已发布），真正的问题是三处旧数据残留 + 界面不解释过滤原因**。

**已修（代码）**：编辑器两个下拉提示被隐藏的已下架数量；「问题场景」页上级分类列标注
「（已下架）」（见 §2.1 / §3.4）。

**旧数据残留（生命周期语义引入前的行，当前门禁下已不可再产生）**：

| 现象 | 事实（08-07 实测） |
|---|---|
| CAT-401 设备问题 / CAT-402、403 物流与优惠（重名两行）本地「已下架」 | 三者 `zendesk_category_ref` 仍在，且线上 Zendesk **均还存在**（旧版下架只改本地标记，早于「下架＝删 Category」语义），与 §2.1 的下架承诺相悖 |
| SCENE-201 SD卡使用问题 / SCENE-202 绑定失败 显示「已上架」 | 父分类 CAT-401 已下架 → 编辑器永远不列它们；线上对应 Section 也仍存在。当前 `requestMetaToggle` 有父分类门禁，此状态无法再造 |
| KB-20004 设备绑定失败排查指南 卡在待审 | 其分类 CAT-401 已下架，审核通过会被门禁拦，需先处置分类 |

**待用户拍板（涉及线上 Zendesk 内容，未擅自动）**：

| 项 | 选项 |
|---|---|
| 设备问题（CAT-401）及其两场景 | (a) 在元数据中心点「上架」——本地恢复、线上目录本就还在，编辑器立即可选；(b) 维持下架并补删线上 Category（不可逆，连带删其下 Section/Article） |
| 物流与优惠重名两行（CAT-402/403） | 决定留哪一行；不留的那行按新语义补删线上 Category（不可逆） |
| KB-20004 | 随 CAT-401 的决定：上架后可正常审核，或驳回该条 |
