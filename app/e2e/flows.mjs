...
// ---------- FLOW-07：总览单库 / 结构树 / 筛选 / 复核 / 章节保护 ----------
{
  ...
  const leafTarget = await page.evaluate(async () => {
    const tree = await (await fetch('/api/kb/tree?libraryId=lib_policy', { credentials: 'include' })).json();
    for (const root of tree) {
      for (const c of root.children) {
        if (c.entries.length > 0) return { chapter: c.name, n: c.entries.length };
      }
    }
    return null;
  });
  let leafCount = 0;
  let leafAfterCollapse = -1;
  if (leafTarget) {
    const row = page.locator('.tree__row--child', { hasText: leafTarget.chapter }).first();
    const caret = row.locator('.tree__caret');
    // 目标章节可能已默认展开，也可能折叠——先确保它是展开态
    if ((await page.locator('.tree__row--leaf').count()) === 0) {
      await caret.click();
      await page.waitForTimeout(400);
    }
    leafCount = await page.locator('.tree__row--leaf').count();
    await caret.click();
    await page.waitForTimeout(400);
    leafAfterCollapse = await page.locator('.tree__row--leaf').count();
    await caret.click();
    await page.waitForTimeout(400);
  }
