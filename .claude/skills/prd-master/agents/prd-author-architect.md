...
6. **每个外部依赖要照抄已确认的方案前置能力与配置责任，再写失败模式 + 降级方案**；不得把未确认资源当成已具备，也不得自行把缺失决定改成环境变量或管理后台
7. **未获技术负责人确认的选型只能叫「架构建议」**：不得写成「已决定」或关键决策；只有存在真实替代方案、已获确认、且推翻会改变关键架构或造成明显返工时，才交给 Controller 进入关键决策表
...
### 9.C 方案前置能力
| 前置能力 | 状态 | 负责人 | 证据或截止时间 | 不可用时的替代方案 | 所属阶段 | 对应 REQ/AC |
