# r3-engineer.md · 工程师主笔章节（R3 修订版，可直接合并终稿）

> 主笔范围（依 r0-chapter-negotiation.md）：§5 详细功能说明（F01–F08）、§7 边界与异常、§9 风险与依赖（实现/发版段）、§12 附录；§10 仅复核性预留。
> 写作口径：只写用户可观察的行为，不写 API/字段类型/协议格式；依赖 G 数据的数值统一写 `[PRD 定数 · G1/G2 后回填]` + 当前建议值；不依赖 G 数据的产品参数标「建议值 · 终稿前确认」（R2 架构师建议采纳）。
> 界面文案（英文）均为**工程侧建议稿**，最终措辞由 PM 副笔在 §5 文案包统一定稿；重叠文案位以 PM 文案包为准，工程新增文案位由 PM 补写后回填（R2-PM 评审第 1 条约定）。全 PRD 用户可见文案唯一落点为本章 §5，§2/§3/§7 只引用不复制。
> 本版依 R2 互审 Controller 裁决（debate-log.md 阶段 5，11 项）修订；处理记录见文末。
...
- **FR 编号（R2-裁决 5）**：本章功能需求采用组级编号 **FR-F01…FR-F08**（与 F01–F08 一一对应，PM §2.4 US 注册表的「由 FR-F0x 实现」引用以此闭合）；需要时可增补子级 FR-F0x-N。编号一经落定入注册表，**只增补不重排**。
...
- 每个 F 结尾标注实现的用户旅程：`[→ PM §2 旅程A/B/C]`（旅程编号随 PM 定稿回填）。

**公共故障态文案位（唯一落点；措辞由 PM 文案包定稿，下为工程侧建议稿；§7.1 引用不复制）**：
...
| 转人工大按钮 | 全宽大按钮（≥44pt） | 隐藏 | 单会话第 2 次拒答后紧随极短说明句出现（R2-裁决 1）；点击进 F03 | 无 |
| 情绪出口置顶卡 | 置顶卡片（双按钮：Talk to a human / Return policy，无任何"优先处理"类承诺，R2-裁决 7） | 隐藏 | 情绪路径触发后置顶常驻，不随消息滚动消失 | 无 |
| 非英语兜底消息 | 消息气泡（英文 + 该语言模板句） | 检测到非英语输入时出现 | 附带转人工入口，照常可转 | 无 |
| 会话菜单（转人工 / 重看首屏按钮 / 结束会话） | 顶部菜单 | 常驻可见 | 对应操作 | 无 |

**交互逻辑**（执行顺序写死，与 [→ 架构师 §3] 决策逻辑逐节点一致，本节只写用户可见结果）：
...
2. 拒答行为（用户可见，R2-裁决 1）：第 1 次拒答 = 常规套话术 + 内联转人工链接；同一会话第 2 次拒答 = **极短说明句（一句话，不重复第 1 次模板）+ 转人工大按钮直接呈现**；此后不再输出拒答类消息——**不存在第 3 次"我没把握"**（连续拒答 ≤2 为硬上限）。
...
5. 事实性解答消息附知识依据，**用户可见形式 = 回答末尾简短来源标注**（如 "Source: Setup Guide"；不展开全文引用、不加跳转承诺，R2-裁决 4）；无可靠依据即拒答，不编造。展示样式细节交设计阶段，评测验收走引用忠实率（[→ PM §8]）。

**文案位清单（措辞由 PM §5 文案包统一定稿；重叠位以 PM 文案包为准，下为工程侧建议稿）**：
...
| 拒答 · 第 2 次（极短说明句 + 大按钮，R2-裁决 1；话术由 PM 文案包落定） | "Let's get a human on this." + 大按钮 [Talk to a human] |
...
- 流式指示：消息发出即显示 responding 指示（无静默空屏，与 §7.1/AC-S-04 同口径）；首条流式内容出现时间为性能预期 `[PRD 定数 · 建议 ≤3 秒]`，不是指示的触发条件。
...
3. It didn't work → 进入显式建模的失败分支；**任何分支路径终点只有两种：解决确认 或 转人工（携带已试步骤）**——无死胡同；分支图校验规则 = **无环，或环上必须存在转人工出口**（R2-裁决 9，与 [→ 架构师 §6] 校验规则一致），图遍历验证为交付验收项。
4. 排障中自由输入：LLM 做意图识别与上下文衔接——追问澄清就地解答后回到当前步骤；情绪触发（关键词或分类器，见 5.2）走 F01 情绪路径（出口置顶）；**与当前场景无关的新知识求解，照常走 [→ 架构师 §3] 决策顺序（含三层拒答）——有依据则答、无依据则拒答，之后询问是否继续排障**（排障态不构成拒答豁免后门）。
...
**文案位清单（措辞由 PM §5 文案包统一定稿；下为工程侧建议稿）**：

| 文案位 | 工程建议稿 |
|------|---------|
| 推进按钮 | "Done" / "It didn't work" |
| 恢复提示条 | "Welcome back! You were on Step 3 — ready to pick up where you left off?" |
...
3. 预计响应时间 = 近期实际首响滚动值或分档展示，禁止硬编码；客服人工侧看到的内容 = 摘要固定字段 + 用户补充 + **会话 ID（呈现为可点开的会话链接）** + **情绪会话标记（内部字段，R2-裁决 7）**；客服首条回复须引用摘要，属 SOP 包验收（[→ PM §9 副笔]）。情绪会话标记不向用户展示、不构成任何用户可见的优先处理承诺——客服 SOP 实际支持优先处理前，界面与话术不得出现 "priority" 类表述。
...
| 文案位 | 工程建议稿 |
|------|---------|
| 摘要卡标题 | "Here's what we'll send to our support team:" |
| 补充输入占位 | "Anything else we should know?" |
| 确认按钮 | "Confirm & transfer"（与 PM 文案包 "Send & connect" 分歧，按约定以 PM 定稿为准） |
...
- 摘要自动生成失败：不阻塞转人工——降级为"会话记录直达客服"+ 用户手填卡点描述；转人工可用性优先于摘要完整性（此降级态的验收 AC 由 PM §10.2 增补，R2 已认领）。
...
- 预计响应时间滚动窗口 `[PRD 定数 · 建议近 7 天首响中位数]`；超时安抚触发 = 超过承诺时间 `[PRD 定数 · 建议 1.5 倍]`（与 PM AC-E-07 同一触发定义）。
...
| 会话评分卡（解决维度） | 轻量评分（"Did this solve your problem?" Yes / No + 可选星级） | 结束判定命中时出现一次 | 提交后显示致谢；Yes/No 进严口径 | 每会话最多出现一次；底线态转接场景不出现（见交互逻辑 1②） |
...
1. 会话结束判定三类，评分触达各异：
   ...
   - ② 转人工完成 → **目标态**在人工会话关闭后触达会话评分卡（问"是否解决"）；**底线态**转接确认时只触达**服务体验类**轻量反馈（不问"是否解决"，R2-裁决 2）——此刻问题尚未被人工解决，问解决与否必得系统性负样本，会污染严口径分子；"是否解决"维度按已确认的行为口径（未转人工且 48h 无新会话）与未答复保守折算处理（与 [→ PM §8] 严口径定义一致）；
...
| 服务体验反馈（底线态） | "Before you go — was I helpful today?" [Yes] [Not really] |
...
**交互逻辑**（回流四触发源 → 清单）：①检索无命中/拒答；②消息级点踩或会话"未解决"标记（含 F02 It didn't work）；③转人工会话；④48h 复问会话。同一问题多次触发按频次聚合为一条，不重复铺行；聚合键口径：有命中条目的按命中条目聚合、无命中的按规范化问题文本聚合（细则归 [→ 架构师 §4.4]，本节引用不复制）。知识条目修订后，变更生效 ≤24h（用户侧可观察：次日同问题得到新答案）。
...
**交互逻辑**：埋点与功能同步交付、同步验收（schema 归 [→ 架构师 §4]，指标口径与目标值归 [→ PM §8]，本节不重复）；接续排障会话不计复问、重新激活会话计复问，口径与 5.1 公共规则一致；灰度对比结论区分"引擎效果"与"入口效果"（entry_source 分层是发版线数据不被混算的唯一保障；entry_source 与交互方式为两个独立维度，枚举见 [→ 架构师 §4]，R2-裁决 8）。

**异常场景**：

- 客户端埋点上报失败（离线/杀进程）：本地暂存补报；看板对延迟数据标注"数据回补中"，不静默缺数。
- 旧版本客户端无 entry_source/app_version 字段：归入"未知入口"桶单列呈报（"未知"为 entry_source 枚举值之一，枚举见 [→ 架构师 §4]），不摊入任何分层（摊入会污染入口分层结论）。
...
1. **文案与 prompt 模板不硬编码语言**：任一用户可见文案与提示模板均从可替换的语言资源加载；可观察结果 = 切换语言资源配置，无需改动对话逻辑本身即可替换全部文案（与 PM AC-C-05 同口径；测试策略归 TDD Master）。
...
2. 错误态入口 → 直达对话并预置对应场景：**先出携带上下文的开场确认，用户确认后才进入对应 F02 流程第一步**（R2-裁决 3，防错误态误报锁死路由）；确认后**用户零打字、零复述故障现场**（建议开场："I can see your camera just went offline. Want me to walk you through getting it back online?"）。
...
| 流式输出中 | 回答逐字输出 | 内容渐进呈现 | — |
| 响应超时 | 超过 `[PRD 定数 · 建议 8–10 秒]` 无内容 | 超时提示 + [Retry] 按钮（文案见 §5.1 文案位 C1） | 重试 / 转人工入口保持可见 |
...
- 索引重建失败分层处置（与 [→ 架构师 §9.B] 统一口径）：上一版索引继续服务（用户无感知）；**中断超 24h 触发运营告警**（对应"变更生效 ≤24h"承诺失守）；**超 48h 未恢复升级呈报**（48h 为「建议值 · 终稿前确认」），不静默带病运行。
...
- 旧版本客户端 + 新引擎（共存主形态）：自由输入问答、三层拒答、转人工（含摘要）、非英语兜底全部可用；按钮组/卡片 UI 缺失时，引擎以纯文本分步引导替代卡片（每条消息一步 + 文字确认），功能降级但路径完整，不硬崩、不出功能黑洞。**此降级即 [→ 架构师 §3.7] "渠道适配层"演进约束的第一个真实实例（旧客户端 = 一种低能力渠道），可作为排障 schema 渠道中立的验收锚点。**
- 新版本客户端 + 未放量引擎（灰度桶外）：入口与 UI 就绪、对话走存量引擎；入口文案两版保持一致（灰度分桶按用户维度，不因界面差异暴露分组）。
- 埋点分层：旧版本无 entry_source/app_version 的流量归"未知入口"桶单列（见 5.7，枚举归 [→ 架构师 §4]），灰度对比结论必须区分"引擎效果"与"入口效果"。
- 发版线覆盖率爬坡本身的风险治理见 §9。
...
- 拒答洪水（薄知识库 + 严拒答）：用户侧可见的抑制机制 = 意图分类豁免 + 连续拒答 ≤2 直接大按钮；运营侧 = 拒答率分层周监控 + 超区间标红 + 灰度放量前置条件（评测集命中率达标且拒答率在区间内，目标值见 [→ PM §8]）——知识库没就绪就不放量，洪水关在小流量里。
...
> 小节编号维持 9.D–9.H，**终局编号待 §9 合稿统一重排**（R2 双方评审均已指出撞号，按 PM 建议本轮各自标注、合稿定终局）。
...
> 工程师仅提交歧义复核清单：逐条检查触发条件唯一、结果可观察、边界数值落占位规范、异常/状态切换场景覆盖 §7 矩阵；不改写场景、不写测试方法（归 TDD Master）。
> R2 已核对项：AC-E-07 超时安抚触发定义与 §5.4 统一（超过承诺窗口，倍数为 PRD 定数 · 建议 1.5 倍）；AC-E-02 与"第 2 次拒答=极短说明句+大按钮"兼容（AC-E-02 只禁第 3 次"我没把握"式回复）；AC-M-01 与错误态"开场确认后进入引导流"兼容（R2-裁决 3）。
...
| FR / US / AC | Functional Requirement / User Story / Acceptance Criteria | 需求追溯三层：US（PM §2.4）→ FR（本章 §5）→ AC（PM §10） |

### 12.6 关联文档

- proposal-v1.md（功能清单与完成标准权威来源）
- scene-anchor.md（场景锚点与前置能力表）
- debate-log.md（Round 3 十二项裁决 + 阶段 5 R2 互审 11 项裁决，本章多处行为规则的裁决出处）
- evidence/complaints.md（差评原话，§5/§7 异常场景与验收用例素材）
- assumptions.md（假设注册表，含 A-007）

---

## ⚠️ 待合稿（R4/Controller）对齐事项

- §5 各 F 的"实现旅程"占位 `[→ PM §2]`：待 PM 旅程编号定稿后回填。
- §5 全部文案位待 PM 文案包定稿回填（重叠位以 PM 为准，工程新增位由 PM 补写）；定稿后 §2/§3/§7 只引用不复制（R0 特殊情况 4）。
- §9 工程段编号 9.D–9.H 与架构段/ PM 段的终局编号由合稿统一重排。
- §7 状态矩阵超时定数（建议 8–10 秒）与 §9.H 首 token 延迟：PRD 定数回填时需与架构师性能预算联动校准。
- §12.3 契约字段与架构师 §4 埋点/会话 schema 的会话 ID 定义、情绪会话标记事件属性需一致（架构师 R3 落 schema）。
- §10 歧义复核清单：R2 已核对三项（见 §10 预留注），其余待 PM §10 R3 定稿后终检。
...
| # | 来源 | 位置/主题 | 标记 | 处理 | 说明 |
|---|------|-----------|------|------|------|
| 1 | PM | §5 建议文案表与 PM 文案包并存 | [需要改] | 采纳 | 各表改为"文案位清单"，声明重叠位以 PM 文案包为准、新增位由 PM 补写回填 |
| 2 | PM | §5.2 第 2 次拒答形态 | [冲突] | 裁决执行（R2-裁决 1） | 改为"极短说明句+大按钮，此后不再输出拒答类消息"；话术由 PM 文案包落定，本稿只留行为与界面元素 |
| 3 | PM | §5.2 自由输入框禁用条件自相矛盾 | [需要改] | 采纳 | 统一为"流式中可输入可发送、入队按序应答"；禁用条件只留空输入/离线 |
| 4 | PM | §5.2 流式指示时机 vs §7.1 | [冲突] | 采纳（以 §7.1 为准） | 消息发出即显示指示；≤3 秒改为首内容性能预期，非指示触发条件 |
| 5 | PM | §5 全章无 FR 编号 | [需要改] | 裁决执行（R2-裁决 5） | 落组级 FR-F01…FR-F08（各 F 标题内声明+5.1 约定+12.1 术语），只增补不重排 |
| 6 | PM | §5.5 底线态评分触达时机 | [OPEN_QUESTION] | 裁决执行（R2-裁决 2） | 底线态转接确认只触达服务体验类反馈、不问"是否解决"；解决维度按行为口径+保守折算；新增服务体验反馈元素行与文案位 |
| 7 | PM | §7.1 三处内嵌用户文案 | [需要改] | 裁决执行（R2-裁决 10） | 三条文案移入 §5.1"公共故障态文案位"（C1–C3，待 PM 定稿），§7.1 改为引用 |
| 8 | PM | §5.4 超时安抚触发 vs AC-E-07 | [冲突] | 采纳（无需改动） | PM 已让步采 1.5 倍并自改 AC 措辞；本稿标注"与 PM AC-E-07 同一触发定义" |
| 9 | PM | §5.3 进度指示 "Step 2 of 5" | [建议] | 采纳 | 改为不带总步数分母的形式（"Step 2"/步骤点）；恢复提示条建议稿同步去分母 |
| 10 | PM | §5.8 F07 删测试方式描述 | [建议] | 采纳 | 删"抽查+演示替换"，只留可观察结果（与 AC-C-05 同口径），策略归 TDD Master |
| 11 | PM | §5.4 摘要生成失败降级 AC | [建议] | 采纳（PM 认领） | PM 在 §10.2 增补异常 AC，本稿仅在异常场景标注已认领 |
| 12 | 架构师 | §5.2 情绪双机制被折叠 | [冲突] | 裁决执行（R2-裁决 11） | 恢复两步分离表述：关键词规则先行（确定性/硬验收 AC-E-03）；未命中→分类器判情绪同走情绪路径（概率性/只进评测集与抽检）；补该概率路径行为定义 |
| 13 | 架构师 | §5.2 输入框矛盾（同 PM 第 3 条） | [需要改] | 采纳 | 同第 3 条，统一"输出中可发送、按序排队应答" |
| 14 | 架构师 | §5.2 第 2 次拒答三方不一致（同 PM 第 2 条） | [冲突] | 裁决执行（R2-裁决 1） | 同第 2 条；不新增话术套系，极短说明句归入 §3 两套体系由 PM 落定 |
| 15 | 架构师 | §5.2 👍/👎 "自由问答"措辞排除预置意图 | [建议] | 采纳 | 改为"挂全部 F01 事实性回答消息（含预置意图路径）；排障卡片/拒答/系统消息不挂"，F04 位置行同步 |
| 16 | 架构师 | §5.2 引用是否用户可见 | [OPEN_QUESTION] | 裁决执行（R2-裁决 4） | 可见但轻量：回答末尾简短来源标注，不展开全文、不加跳转承诺；样式交设计阶段 |
| 17 | 架构师 | §5.3 排障中知识求解须走三层拒答 | [需要改] | 采纳 | 改为"照常走 §3 决策顺序（含三层拒答），之后询问是否继续排障"，封死豁免后门 |
| 18 | 架构师 | §5.4/§12.3 会话链接 vs 会话 ID 术语 | [建议] | 采纳 | 统一为"会话 ID（呈现为可点开的会话链接）"，两处同步 |
| 19 | 架构师 | §5.6 聚合键定义缺失 | [建议] | 采纳 | 标注聚合口径（命中条目/规范化问题文本）并引用架构师 §4.4 细则 |
| 20 | 架构师 | §5.7 "未知"枚举越界 | [需要改] | 采纳 | 保留行为描述，加"（枚举见 §4）"引用；枚举扩充由架构师在 §4.2 落 |
| 21 | 架构师 | §5.9 错误态直达 vs 开场确认 | [冲突] | 裁决执行（R2-裁决 3，无需改动） | 裁决采本稿方案；仅在交互逻辑 2 显式标注"先确认再路由"与裁决号 |
| 22 | 架构师 | §7.4 48h vs 架构师 §9.B 24h | [冲突] | 采纳（分层统一） | 改为 24h 触发运营告警、48h 未恢复升级呈报，与架构师 §9.B 同口径 |
| 23 | 架构师 | §9 小节编号撞车（同 PM 评架构师稿相关条） | [需要改] | 部分采纳 | 按 PM 建议维持 9.D–9.H 并标注"终局编号待合稿重排"；不自行改号避免交叉引用返工 |
| 24 | 架构师 | §5 全章 FR 编号（同 PM 第 5 条） | [需要改] | 裁决执行（R2-裁决 5） | 同第 5 条，采组级方案（PM 现稿零改动） |
| 25 | 架构师 | 非 G 依赖参数混用 PRD 定数占位 | [建议] | 采纳（按所列六处执行） | 1,000 字符/500 字符/200 条/循环 2 次/超时率 5%/告警 48h 改标「建议值 · 终稿前确认」；12.1 术语表补两类占位区分；1.5 倍安抚等未列项维持原占位待合稿统一 |
| 26 | 架构师 | §7.6 加 §3.7 渠道适配层对应标注 | [建议] | 采纳 | §7.6 旧版本降级段补一句演进约束落地锚点标注 |

**统计**：26 条（PM 11 + 架构师 15）——裁决执行 8 条、采纳 17 条（含 2 条"无需改动"与 1 条部分采纳）、驳回 0 条。另按裁决 7/9 主动同步：§12.3 契约增补第 6 字段"情绪会话标记"；§5.3 分支图校验规则补"无环或环上必有转人工出口"。
