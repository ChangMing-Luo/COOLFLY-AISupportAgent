...
> 作者：prd-author-pm ｜ 日期：08-03-2026 ｜ 输入：proposal-v1.md（含已确认 MVP 划分）、scene-anchor.md、assumptions.md、debate-log.md、evidence/{competitors,complaints,benchmark}.md ｜ 分工依据：drafts/r0-chapter-negotiation.md
> 主笔章节：§0 / §1 / §2 / §8 / §10 / §11；文末附 PM 副笔供稿（§5 用户可见文案包、§9 商业风险与外部依赖段）供各章主笔合并。
...
>
> 未入账弹性：店铺评分 3.6–4.0 → 4.3+ 对转化率的拉动（3→4 星下载/转化 +89%，Apptentive 下载口径，店铺购买转化为类比推断，依据等级 C，来源：benchmark.md）；技术定位省来回若单列约 ¥3–5 万/年（工程师工时，独立资源）。
...
flowchart LR
    subgraph 入口
        E1[错误态 Get help<br/>携带故障现场] --> S
        E2[固定一级入口] --> S
    end
    S[首屏：零打字按钮组<br/>+ 自由输入] -->|按钮| J1[旅程 A · 引导排障<br/>一步一屏 Done / It didn't work]
    S -->|自由文本| IC[意图分类<br/>细节见 §3]
    IC -->|知识求解| QA[知识问答]
    IC -->|情绪/退款关键词| J3[旅程 C · 出口置顶<br/>转人工 + 退货政策]
...
北美用户 Sarah 配对到一半卡住，配对失败页直接显示 "Get help" → 点进去首屏就有 "Pairing failed" 按钮，不用打字 → 分步引导第一步问"你连的是 2.4GHz 还是 5GHz？"配路由器示意图 → Sarah 切出 App 去改路由器设置（超过 30 分钟也没关系）→ 回来自动恢复到当前步骤 → 点 "Done"，配对成功 → AI 确认解决，弹出轻量评分 → 全程 8 分钟，零打字也零等待。（2.4GHz/5GHz 为品类第一装机杀手，来源：complaints.md 痛点 1）
...
用户折腾 2 小时后打出 "this thing is garbage, I want a refund" → 关键词规则命中，不进拒答判定、不追问技术细节 → 先共情，出口置顶：转人工（优先接入）+ 退货政策入口 → 若用户愿意，再给"最后试一步"最短排障路径。（机制细节与验收边界见 §3；关键词触发为确定性硬验收，怒气识别只进评测集与抽检）

### 2.4 用户故事（US 注册表 · ID 与 F01–F08 对齐）

> FR 编号由 §5 工程师主笔分配；本节按"每功能一组 FR"约定引用 FR-F0x，若 §5 最终采用更细粒度编号，R2 对齐（见文末边界备忘）。
...
**F05 知识库回流闭环（间接角色）**
- **US-F05-01**：作为知识库内容运营（人工客服侧兼任，间接角色），我每周拿到一份按频次排序的待补条目清单（含原始问题/命中文档/会话链接），知道该优先补什么。（由 FR-F05 实现）
...
|------|------|------|------|--------|--------------|
| 自助解决率（宽口径） | 未转人工 且 48h 内无任何新会话；接续排障会话不计新会话；已结束会话重新激活并产生用户消息=新会话 | 现有 AI 解决率 `[PRD 定数 · G1 后回填]` | 高频类 60% / 整体 50%（3 个月，假设值 A-004） | 周报 | F06 看板，**按 entry_source（固定入口/错误态入口）分层呈报** |
| 自助解决率（严口径，验收用） | 宽口径 ∩ F04 显式确认（未答复保守折算） | 无（新口径） | 高频类目标 60% / 底线 45% | 3 个月 | F06 看板；连续 2 个月低于 45% 且回流机制正常运转 → 触发切采购扳机（见 8.3） |
...
| 高频误拒率 / 连续拒答 | 离线评测集反向指标 | — | 误拒率上限 `[PRD 定数 · G1/G2 后回填]`；单会话连续拒答 ≤2 次 | 每次发版前评测 | 评测集（构成见 §4） |
...
| 转人工首次响应 | 人工渠道首响 | 现状人工首响（G1 估算） | **不劣于现状**（不变行为，见 §10.5） | 持续 | F06 |
| 会话满意度评分 | F04 会话级轻量评分 | 无 | 趋势参考，不设达标线（已裁决，B11） | 周报 | F06 |
| 语言分布 | 来话语言占比 | `[PRD 定数 · G1 后回填]` | 非英语 >15% → 触发提前完整多语言评估（A-002 传感器） | 月报 | F06 |
...
- **灰度对比结论必须区分"引擎效果"与"入口效果"**：错误态入口流量问题更难、情绪更差，混算会让新系统被自己的增量流量拉低（来源：debate-log.md 裁决 2）。
...
|----|--------------------|--------------------|
| AC-M-01 | 用户在配对失败错误页点 "Get help" | 进入客服对话；首条上下文中可见故障场景已自动带入（用户无需描述"我在配对时失败了"）；会话开场可见 AI 身份披露语（文案见 §5） |
| AC-M-02 | 用户在对话首屏不打字，点 "Pairing failed" 按钮 | 直接进入配对排障分步引导第一步；一步一屏，卡片内仅 "Done" / "It didn't work" 两个操作按钮（无 👍/👎） |
| AC-M-03 | 用户在引导步骤点 "Done" 且为最后一步 | AI 给出解决确认，随后出现会话级轻量评分入口 |
| AC-M-04 | 用户点首屏 "Membership" 按钮 | 直接进入会员知识问答（预置意图，跳过打字），AI 给出基于知识库的会员解答 |
| AC-M-05 | 用户自由输入英文提问四类高频问题（安装/联网/配对/会员）之一 | AI 给出基于知识库的英文回答；事实性解答均有知识依据（引用机制见 §3，不在本章展开）；四类高频问题全流程可问可答 |
...
| AC-M-08 | 用户对 AI 单条回答点 👎 | 出现原因选择（不相关/看不懂/试了没用），提交后界面确认收到；该消息进入回流清单（清单可观察结果见 AC-M-09） |
| AC-M-09 | 运营（间接角色）在上线后任一周查看回流清单 | 可导出一份按频次排序的待补条目清单，字段含原始问题/命中文档/会话链接/频次 |
| AC-M-10 | 业务负责人（间接角色）在灰度第一周查看看板 | 双口径解决率、拒答率（分层）、转人工量、语言分布均有数据、按周可看、按 entry_source 分层 |
| AC-M-11 | 新用户从 App 首页出发找客服 | 3 步以内到达固定客服入口（验收用例取自差评原话"labyrinthian maze"，见 §1.1） |
...
| AC-E-02 | 同一会话内 AI 第 2 次拒答 | 界面直接呈现转人工大按钮；不出现第 3 次"我没把握"式回复 |
...
| AC-E-07 | 转人工后超过承诺时间未有人工回复（目标态） | 用户收到主动安抚消息（含超时联系方式）；不发生"承诺了通知却永远沉默" |
...
| AC-C-02 | 关键操作按钮（Done / It didn't work / 转人工等） | 触达区域 ≥44pt |
| AC-C-03 | VoiceOver 开启走完主流程（进入入口→提问→转人工） | 关键控件可被读出、可操作（基础走查级，及格线不下放） |
| AC-C-04 | 未升级到含 F08 的 App 旧版本用户 | 仍可从原有固定入口使用引擎线全部能力（问答/排障内容/转人工）；仅缺错误态入口与新按钮组 UI；覆盖率随升级率如实呈报，不算入引擎灰度承诺 |
...
| AC-I-03 | **转人工后人工首响不劣于现状** | 转人工请求到人工首次回复的时长，不劣于上线前人工渠道水平（基线 `[PRD 定数 · G1 后回填]`） |
| AC-I-04 | **App 现有功能不受影响** | 客服模块任何故障（LLM 不可用、引擎超时）不影响 App 其余功能（设备直播、通知等）正常使用 |
| AC-I-05 | **不虚假承诺** | 底线态转人工话术中不出现"回复会通知你"字样；预计响应时间为滚动实际值或分档展示，界面上不出现硬编码的固定承诺时长 |
...
| A-001 | 飞书知识库质量足以支撑高频回答（第一命门，🟠） | G1 后排序改造清单；改造完成=schema 校验+评测集命中率 ≥80%；灰度放量与知识库就绪联动 |
| A-002 | 第一版英文单语可接受（依据等级 B） | F06 语言分布实测，非英语 >15% 触发提前多语言 |
| A-006' | "高频三类占咨询量 60%+"（A-006 号目，风险🟡） | G1 验证，**<40% 触发回炉线** |
...
| 依据 | 关键数字/结论 | 来源 |
|------|--------------|------|
| COOLFLY 自身差评（逐字原文） | 掉线无指引、"labyrinthian maze" 导航噩梦 | App Store 评论页（complaints.md 痛点 6/9，2026-08-03 抓取） |
...
> R0 铁律：所有用户可见 UI 文案唯一落点在 §5，§2/§3/§7 只引用不复制。以下为 PM 副笔的完整文案稿（英文为交付语言），工程师合并进 §5 界面元素表；措辞如与界面元素表结构冲突，以本文案内容为准、以工程师的表结构为准。
...
**4. 拒答话术 · 常规套（F01 第 1 次没把握）**：
> "I don't want to guess on this one — I couldn't find a reliable answer in our guides. Let me connect you with our support team; they'll already have your conversation and device info."
> （第 2 次拒答时不再输出模板句，直接呈现转人工大按钮：`Talk to a human`）
...
> 置顶按钮：`Talk to a human (priority)` ｜ `Refund & return policy`
...
> ② "You can close this page — we'll notify you here when they reply."（发送前完成场景化通知权限请求；被拒则改为："Check back here anytime — your conversation will be waiting."）
> ③ 超时安抚（超过承诺时间自动下发）："Sorry for the wait — our team is still on your case. If it's urgent, reach us at {support_email}."
...
**9. 转人工摘要确认界面（F03）**：标题 "Here's what we'll send to our team"；字段按交接契约顺序（已试步骤/SN/型号/卡点，见 §12 附录）；编辑入口 `Add details`；确认按钮 `Send & connect`。

---

## 【PM 副笔供稿 · 供 §9 主笔合并】商业风险与外部依赖段

> §9 三方各写各段：架构师=技术/架构风险，工程师=实现/发版风险，本段=PM 的商业风险与外部依赖。

### 9.PM.1 G1–G4 前置门槛表（硬窗口 ≤2 周 · 与 PRD 编写并行 · 回退规则写死）

| # | 动作 | 关键产出 | 负责人 | 硬截止 | 超期/回退规则（写死） |
|---|------|---------|--------|--------|----------------------|
| G1 | 现有 AI 客服历史数据导出 + 基线分析 | 咨询分布、现有解决率估算、转人工原因 Top10、退货-咨询交叉比例、语言分布、3 人团队月实际处理量 | 用户团队导出，AI 分析 | ≤5 个工作日 | 超 2 周未产出 → 视同拍板 3 自动落保守版 C；**回炉线**：高频三类占比 <40% 或退货-咨询交叉 <30%，任一触发回评审团复审；最小字段缺 ≥2 项 → 如实告知企业家双档目标为纯假设后再拍 |
...
### 9.PM.5 合规 NFR 前置项（上线前置，🟩，详见 §8 合规节/架构师章节）
...
- **US 的 FR 引用粒度**：本稿按"每功能一组"引用 FR-F01…FR-F08；若工程师 §5 采用 FR-F01-01 细粒度编号，R2 时我把每条 US 对齐到具体 FR 编号（check_consistency 格式要求）。
- **§10 vs §5/§7**：AC-E-05（无死胡同）、AC-S-01～S-07（状态矩阵）只写了产品可观察结果，触发条件的精确定义（超时秒数最终定数、静默判定实现口径）以工程师 §7 为准，R2 核对不打架。
- **§10 vs §3**：AC-E-01/E-06 引用"三层拒答/硬规则范围"，层定义归架构师 §3；若 §3 对豁免类别措辞与 AC-E-03 有出入，以 §3 机制 + 本章可观察结果合并对齐。
- **文案包落点**：本稿文案全部作为 §5 副笔供稿，工程师合并后 §2/§3/§7 一律只引用；若工程师界面元素表新增了我未覆盖的用户可见文案位（如空态、错误 toast），R2 时我补写。
- **§8 与 §4 的口径/schema 分界**：§8 只写口径与目标值，事件 schema 字段归架构师 §4；R2 核对 entry_source/channel/app_version 字段名一致。
- **§9.PM 段与架构师/工程师风险段**：G1–G4 表由我落，若架构师在 A-001/拒答洪水段重复引用门槛表，R2 时合并为单一落点互相引用。
