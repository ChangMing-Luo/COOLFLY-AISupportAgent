...
| RA-016 | P2 | REQ-F09-04、REQ-F09-19 | 约束冲突（边界条件） | 四眼原则要求在任审核员 ≥2 人，而系统管理员可禁用用户——若第二审核员被禁用，发布流程死锁的界面处置未在 PRD 显式写死 | 组织侧缓解已登记：RBAC 角色到人前置行 Fallback「审核员不足 2 人→发布动作暂缓上线」；建议 PRD 终稿在视图⑩补禁用告警提示（当禁用操作将使某角色在任人数低于下限时提示后果） | open |
