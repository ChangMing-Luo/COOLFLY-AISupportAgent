...
| **语言字段** | F07 约束，字段必须存在；条目语言在录入时选择（08-04-2026 第三轮拍板修订：不再限定恒为英文，服务 App 英文用户的生产条目为英文）。**检索语言过滤（随修订同步的安全约束）**：对话引擎检索按会话渠道语言过滤——App 英文渠道仅命中英文条目，非对应语言条目不进入候选（防中文条目被引用给英文用户） | 架构约束（§3.7）+ 内容运营录入时选择 |
