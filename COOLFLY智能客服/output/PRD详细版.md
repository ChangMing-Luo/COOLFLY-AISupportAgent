...
| F09 | 知识库管理台（知识库唯一权威源：条目管理/录入导入/AI 整理/会话提炼审核/发布门禁，细节见 §5.10） | 应做 | 🟩（不卡六项及格线验收，同 F05 待遇） | 引擎线（内部 Web，不依赖 App 发版） | 阶段一 MVP（08-04-2026 拍板增补） |
...
- **US-F09-01**：作为知识库内容运营，我要在管理台录入或导入知识条目（手工表单、飞书一次性迁移、Markdown/CSV 批量导入），并让 AI 给出切条、问题标签与场景/型号归类建议供我确认，让 120+ 篇存量与新增内容快速变成可检索的结构化条目。（由 FR-F09 实现）
- **US-F09-02**：作为知识库内容运营，我要在审核队列里看到 AI 从真实会话聚类起草的条目草稿（含来源会话与频次、标签建议），一键通过发布/驳回/改后发布——未经我审核的草稿绝不生效。（由 FR-F09 实现）
- **US-F09-03**：作为知识库内容运营，我确认后的问题标签要成为检索信号，让用户的各种问法更准命中条目；发布须过评测集回归，回归不过的条目不生效，命中率仍以 §8 目标值验收。（由 FR-F09 实现）
- **US-F09-04**（08-04-2026 第二轮拍板增补）：作为知识库内容运营，我要在管理台效果看板看到每条知识的使用与解决效果（命中/引用/点踩/条目准确率），知道该优先修哪条。（由 FR-F09 实现）
...
**交付约束**：埋点与功能同步交付、同步验收（缺埋点的功能不算完成）；事件语义一经发布只增补不改义，防口径断代。**F09 效果看板零新增埋点（08-04-2026 第二轮拍板）**：F09 管理台效果看板（§5.10）全部指标由上述既有事件聚合产出——「回答消息」事件的引用条目标识列表（引用采纳计数）+「消息级反馈」（点踩与「试了没用」原因）+「会话结束」（解决口径归因），不新增任何埋点事件；口径唯一落点 §8.2。
...
flowchart LR
    ...
    T3["触发源③ 转人工会话"] --> Q
    T4["触发源④ 48h 复问会话"] --> Q
    Q --> DR["AI 聚类并起草条目草稿<br/>（含问题标签建议 / 来源会话 / 频次）"]
    DR --> RQ["管理台审核队列（F09 §5.10）"]
...
回流机制、管理台审核队列（F09）与清单导出属 MVP 即上线；仅周消化 SLA 挂拍板 2 名单（08-04-2026 拍板：回流消化工作面从「导出文件」升级为管理台内审核队列，周度导出保留为导出能力）。
...
- **管理台 = 知识库唯一权威源**：飞书 120+ 篇存量文档一次性导入为初始数据（导入内容进「待整理」队列，迁移兜底见 §9.2）；此后条目的录入、修订、审核、发布全部在管理台完成，**不做与飞书的双向同步**（防口径分叉）。
- **界面语言 = 简体中文（08-04-2026 第二轮拍板强化）**：第一版管理台**全部界面元素（导航/按钮/标签/提示/占位文案）100% 简体中文**，不残留英文装饰性文案（品牌词 "COOLFLY" 允许保留）；本节文案定稿即中文界面文案唯一落点。**界面语言与内容语言分离（显式约束）**：知识条目内容本身保持英文——服务北美 App 用户，条目带语言字段（§4.1 既有约束）——界面文案全中文不改变条目内容语言，不违反 F07 约束。演进注记：管理台 UI 多语言切换为后续增强——复用 F07 约束一「文案不硬编码语言」，管理台 UI 文案同样从语言资源加载，第一版仅提供简体中文资源，后续加语言 = 加资源包、不改界面逻辑。

**界面元素与展示规则**：

| 元素 | 类型 | 默认态 | 操作后 | 禁用条件 |
|------|------|--------|--------|---------|
| 条目列表 | 表格（搜索框 + 筛选器：场景/型号/语言/状态/问题标签） | 按更新时间降序展示全部条目 | 搜索/筛选即时过滤；点行进入编辑 | 无 |
| 条目编辑器 · 问答型 | 表单（标题/场景/型号/语言/正文/问题标签，字段对齐 §4.1 schema） | 新建为空 / 编辑载入现值 | 保存生成修订记录 | 无 |
| 条目编辑器 · 排障流程型 | 结构化编辑器（步骤节点 + Done/It didn't work 分支，对齐 §4.1 排障流程 schema） | 同上 | 保存时执行无死胡同图遍历校验（§3.6），不过不允许发布 | 无 |
| 状态流转控件 | 状态操作（草稿 → 已上线 → 待改造 / 已下线） | 显示当前状态 | 流转留痕；转入「已上线」须过发布门禁（见交互逻辑 5） | 未过审核/回归的条目不可流转为已上线 |
| 修订历史 | 时间线（修订人/时间/变更摘要） | 每条目可查 | 可查看任一历史版本 | 无 |
| 手工录入表单 | 表单入口 | 常驻 | 提交进「待整理」队列或存为草稿 | 无 |
| 批量导入 | 文件上传（Markdown / CSV；飞书一次性迁移走同一入口） | 常驻 | 导入内容全部进「待整理」队列，逐条报告结果 | 无 |
| AI 整理建议卡 | 建议卡（切条建议 / 问题标签建议 / 场景与型号归类建议） | 对录入/导入内容自动生成，**建议态** | 运营逐项采纳/修改/拒绝后方生效 | AI 服务不可用时隐藏并明示（见异常） |
| 会话提炼审核队列 | 队列列表（AI 起草草稿：正文 + 问题标签建议 + 来源会话链接/脱敏摘要 + 频次），按频次降序 | 常驻，待审数量可见 | 单条打开后三操作：通过发布 / 驳回 / 改后发布 | 无 |
| 发布门禁结果面板 | 面板（评测集回归结果 + 留痕记录） | 发布动作后展示 | 回归通过→条目生效；失败→条目回炉并展示失败用例 | 无 |
| 周清单导出 | 导出按钮 | 常驻 | 导出任意周区间待补清单（字段同 §4.4） | 无 |
| 效果看板 · 场景覆盖卡 | 统计卡（管理台第 5 导航区「效果看板」，08-04-2026 第二轮拍板增补） | 已上线条目数按场景分布 + 覆盖场景数，对照 G1 咨询分布场景集 | 只读 | G1 前降级态：按 §4.1 现有场景分类展示并标注「场景集待 G1 回填」（见异常） |
| 效果看板 · 使用频率卡 | 统计卡（同上导航区） | 条目被检索命中次数，周趋势 | 只读 | 无 |
| 效果看板 · 引用采纳卡 | 统计卡（同上导航区） | 条目被实际引用进回答的次数，周趋势；**与命中分列——命中未被采纳的差值可见** | 只读 | 无 |
| 效果看板 · 知识库准确率卡 | 统计卡（同上导航区） | 知识库整体条目准确率 + 周趋势（口径唯一落点 §8.2「知识条目效果」行） | 只读 | 无 |
| 效果看板 · 条目效果列表 | 表格（逐条目：命中数/引用数/点踩数/「试了没用」数/条目准确率） | 默认按条目准确率升序（最差条目浮顶） | 点击条目行跳转该条目详情（既有编辑器与修订历史）；可切换排序列 | 引用数低于样本量下限的条目准确率列显示「样本积累中」（见异常，AC-E-15） |

**交互逻辑**：

1. **条目管理**：列表支持按场景/型号/语言/状态/问题标签组合筛选与搜索；问答型与排障流程型分别用对应编辑器；状态流转 = 草稿 → 已上线 → 待改造 / 已下线，每次修订与流转留痕（修订人/时间/变更摘要），历史版本可查。
2. **录入与导入**：手工表单逐条录入；批量导入支持飞书一次性迁移与 Markdown/CSV 文件；导入内容一律先进「待整理」队列，不直接上线。
3. **AI 自动整理（全部建议态）**：对录入/导入内容自动给出①切条建议（长文拆为多条目）②问题标签（用户问法变体，多值）③场景/型号归类建议；运营逐项采纳、修改或拒绝后方生效——拒绝不阻塞人工直接编辑发布（AC-E-13）。
4. **会话提炼审核队列（F05 消化工作面，§5.6 引用本节）**：F05 回流触发源会话由 AI 聚类并起草条目草稿（含问题标签建议、来源会话链接或脱敏摘要、频次），进审核队列按频次降序排列；运营三操作 = 通过发布 / 驳回（须填原因）/ 改后发布；**无人审不生效（铁律，08-03-2026 拍板确立、08-04-2026 随 F09 提前落地）**：未经审核的草稿不进检索库、不被 F01 命中（AC-M-15）。
5. **发布门禁**：任何条目转入「已上线」（含改后发布）时触发 §4.3 评测集回归，结果留痕可追溯；回归不过 → 条目回炉（不生效，失败原因与用例可见，AC-E-14）；回归通过 → 发布生效、索引更新，**变更生效 ≤24h 承诺不变**（§4.4）。
6. **检索联动**：人工确认后的问题标签作为 F01 检索信号（扩充召回 + 过滤），让同一条目命中更多用户问法变体；**命中率仍以评测集验收，§8 目标值不变、不新增指标**（AC-M-13 验收录入→发布→检索命中闭环）。
7. **知识库效果看板（管理台第 5 导航区 · 08-04-2026 第二轮拍板增补）**：四卡（场景覆盖/使用频率/引用采纳/知识库准确率）与条目效果列表均为**只读**呈现；指标口径唯一落点 §8.2「知识条目效果」行，本节引用不复制；数据与 F06 业务看板**同源埋点聚合（§4.2 既有事件，零新增埋点），不重复建设**——F06 看会话与业务结果，本看板看条目供给侧效果，呈报位置区隔。条目效果列表默认按条目准确率升序——**最差条目浮顶，直接指向该优先修订哪条**；条目准确率低于标黄线「建议值 60% · 终稿前确认」的条目标黄提示优先修订（与 F05 回流联动：标黄条目即修订候选）；点击条目行跳转条目详情（既有编辑器与修订历史，不新增编辑入口）。
...
| 导入结果提示 | 「导入完成：{n} 条进入待整理队列；{m} 条失败（逐条原因见下）。」 |
| 并发编辑冲突提示 | 「该条目已被他人更新，请基于最新版本重新修改（你的改动未覆盖线上内容）。」 |
| 效果看板样本不足提示 | 「样本积累中」（引用数不足样本量下限时代替准确率数值显示，AC-E-15） |
| 效果看板 G1 前降级标注 | 「场景集待 G1 回填」 |

**异常场景**：

- **发布门禁回归失败**：条目不生效、状态回炉（回草稿），失败原因与失败用例在结果面板可见；不存在"回归失败但条目已上线"的中间态。
- **批量导入部分失败**（格式错误/字段缺失）：成功条目照常进「待整理」队列，失败条目逐条报告行号与原因；不整批静默失败、不静默丢行。
- **AI 整理/起草服务不可用**（LLM 故障）：建议卡隐藏并明示 AI 建议暂不可用；手工录入、编辑、审核、发布全部照常可用——AI 能力为增强不是依赖（与 §7.5 降级原则同源）。
- **审核队列积压**：按频次降序保持可操作；周消化 SLA（Top 10）照常按队列数据判定（§5.6）；积压不影响已上线条目正常服务。
...
- **效果看板数据延迟/回补**（埋点上报延迟）：延迟数据标注「数据回补中」，不静默缺数（与 F06 看板同口径，§5.7）。
...
- 审核队列排序 = 来源会话频次降序；周消化 Top 10（口径同 §5.6/§4.4）。
- 发布生效时限 ≤24h（既有承诺，§4.4）。
- 条目准确率标黄线 =「建议值 60% · 终稿前确认」（效果看板，口径归 §8.2）。
- 条目准确率样本量下限 =「建议值 引用 10 次 · 终稿前确认」（低于下限显示「样本积累中」）。
- 效果看板数据粒度 = 周（与 §8.2「知识条目效果」行一致）。
...
| 知识条目效果（F09 管理台看板，08-04-2026 第二轮拍板增补·口径唯一落点） | 场景覆盖=已上线条目按场景分布与覆盖场景数（对照 G1 咨询分布场景集，G1 前按 §4.1 现有场景分类降级）；使用频率=条目被检索命中次数；引用采纳=「回答消息」事件·引用条目标识列表计数（§4.2 既有事件，零新增埋点），与命中分列——命中未采纳可见；**条目准确率=引用该条目的事实性回答中，未被点踩且所在会话按宽/严口径判解决的比例**（双口径同上两行解决率；归因性质=消息级近似归因——一次会话可引用多条目，看板如实标注该性质）；「试了没用」点踩数单列（答了未解决的直接信号，不并入普通点踩） | 无（新口径） | 条目准确率 <「建议值 60% · 终稿前确认」→ 管理台标黄提示优先修订；引用数 <「建议值 10 次 · 终稿前确认」不计算准确率、显示「样本积累中」 | 周粒度 | F09 管理台效果看板（§5.10）；与 F06 业务看板同源埋点聚合、不重复建设 |
...
| 飞书知识库 API（08-04-2026 拍板改为一次性） | **初始一次性迁移导入**：120+ 篇存量文档导入 F09 管理台「待整理」队列；此后管理台为知识库唯一权威源，不做双向同步（§5.10） | 用户团队（API 权限前置登记） | 用户团队申请 / 技术团队接入 | 服务端 | MVP（一次性：迁移完成即生命周期结束，不留同步链路） | 迁移失败 / 权限未就绪 → **人工从飞书导出文件（Markdown/CSV），经 F09 批量导入兜底**（§5.10）；迁移后知识变更生效 ≤24h 由「管理台发布 → 索引更新」承载（§4.4），索引失败分层告警归 §7.4（唯一落点），与飞书无关 |
...
| AC-M-15 | F09 管理台内存在未经审核的草稿/待整理条目，App 用户提出该条目覆盖的问题 | 未审核条目不被检索命中、不出现在任何回答引用中；只有审核通过并过发布门禁的条目进入检索库（无人审不生效铁律，§5.10） |
| AC-M-16 | 知识库内容运营或业务负责人打开 F09 管理台效果看板（第 5 导航区，08-04-2026 第二轮拍板增补） | 四卡可见：场景覆盖（已上线条目场景分布+覆盖场景数）/使用频率（检索命中周趋势）/引用采纳（实际引用进回答周趋势，**与命中分列——命中未被采纳可见**）/知识库准确率（整体+趋势）；条目效果列表可见：逐条目命中数/引用数/点踩数/「试了没用」数/条目准确率，可按准确率升序排列（最差条目浮顶）；全部指标口径与 §8.2「知识条目效果」行一致（§5.10） |
...
| AC-E-13 | 知识库内容运营（内部角色）对 AI 给出的切条/问题标签/归类建议逐项修改或拒绝（F09 管理台） | 修改后按运营修改内容生效；拒绝后该建议不生效，且不阻塞运营人工直接编辑与发布（AI 建议为建议态、非门槛，§5.10） |
| AC-E-14 | 运营在 F09 管理台发布条目时评测集回归失败 | 条目不生效（不进检索库、状态回炉），管理台内可见回归失败原因与失败用例；修订后可重新发布（§5.10 发布门禁） |
| AC-E-15 | F09 效果看板中某条目引用数低于样本量下限（「建议值 10 次 · 终稿前确认」，08-04-2026 第二轮拍板增补） | 该条目**不显示准确率数值**，准确率位置显示「样本积累中」——不显示误导性准确率；其余指标（命中数/引用数/点踩数/「试了没用」数）照常显示（§5.10） |
...
| US-F01-01 … US-F09-04 | 23（F01×7、F02×3、F03×2、F04×2、F05×1、F06×2、F08×2、F09×4；F07 无 US，已在 §2.4 显式声明） | §2.4 | 每条 US 均落 FR；F05/F06 间接角色 US 与 §10 AC-M-09/M-10 对应；F09 四条 US（角色=知识库内容运营，§2.1 已增补该直接角色；US-F09-04 为 08-04-2026 第二轮拍板增补）与 AC-M-13–16/AC-E-13–15 对应 |
| AC-M | 16（AC-M-01–16） | §10.1 | 引用的文案（§5）、维度定义（§4）、机制（§3）均有落点；AC-M-13–15（F09，08-04-2026 增补）↔§5.10 发布门禁/审核队列/无人审不生效、§4.1 审核信息；AC-M-16（F09 效果看板，08-04-2026 第二轮拍板增补）↔§5.10 效果看板、§8.2「知识条目效果」口径行 |
| AC-E | 15（AC-E-01–15） | §10.2 | AC-E-02↔§3.3 连续计数；AC-E-03↔§3.1 关键词规则（清单 v1 §12.7，R5-T1）；AC-E-06↔§5.2 超范围专属文案（R5-S3）；AC-E-07↔§5.4 缓冲系数定数；AC-E-09↔§5.4 摘要降级；AC-E-11↔§5.9 降级行为；AC-E-13/14（F09，08-04-2026 增补）↔§5.10 AI 建议态与发布门禁；AC-E-15（F09 效果看板样本量下限，08-04-2026 第二轮拍板增补）↔§5.10「样本积累中」，均闭合 |
...
| 条目准确率标黄线（F09 效果看板，08-04-2026 第二轮拍板增补） | 60% | §5.10 / §8.2 |
| 条目准确率样本量下限（F09 效果看板，08-04-2026 第二轮拍板增补） | 引用 10 次 | §5.10 / §8.2 / AC-E-15 |
