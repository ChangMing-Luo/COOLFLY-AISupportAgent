...
| **知识审核员** | 质量与发布把关人（如李骁） | **直接（中台）** | 守住质量线：审核、发布、同步、回滚的唯一执行者 | 审核通过/驳回（理由必填）；发布并同步 Zendesk；版本回滚；drift 处置（覆盖/拉回）；同步失败重试；也可编辑与提交 | 全部知识库；**四眼原则**：自己提交的条目须另一名审核员通过；查看全量审计日志 | 阶段一 MVP |
...
运营王雯在工作台看到「被驳回待改」1 条——「会员退订与计费周期」被李骁驳回（理由：计费周期与财务口径不符，请附财务确认截图）。她按意见补充财务确认与跨月退订示例后重新提交；审核员李骁在审核中心看到重提条目与完整审核历史，通过后触发发布门禁（格式字段完整性/内部段落标记/英文版本状态/代理评测）四查通过，条目入同步队列，几分钟后帮助中心「会员计费」Section 下文章更新，客服 Knowledge 面板同步可见。全程每一步写入操作日志。
...
- **US-F09-09**：作为知识审核员，我要在审核中心统一处理四来源（AI 挖掘/人工录入/版本修订/反馈修订）的待审条目，看到变更前后对照与审核历史；驳回必须填写理由，理由回传提交人并留痕。（由 FR-F09 实现）
- **US-F09-10**：作为知识审核员，我通过审核后系统自动跑发布门禁（格式字段完整性/内部段落标记/英文版本状态/代理评测），全过才入同步队列；同步失败自动重试后告警、可手动重试，英文未校验等原因的阻断可见原因。（由 FR-F09 实现）
...
> 本章为概念级：只定义"有什么信息、谁产生、流向哪"，不定义字段物理类型。指标口径与目标值归 §8。原 §4.2 App 埋点 schema、§4.3 离线评测集（golden set）、§4.4 App 触发源回流数据流随 F01–F08 **废止**——评测集机制降级为发布门禁的**代理评测**（本台内部向量检索验证，如实标注：代理指标≠Zendesk AI 真实表现，见 §4.4/§5.10）。
...
| **向量化状态** | 未向量化 / 向量化中 / 已向量化 / 待重建 / 向量失败；**向量只服务本台内部**：挖掘查重、缺口聚类、发布门禁代理评测——Zendesk AI 有自己的索引，两者独立 | 系统（内容变更置「待重建」） |
...
- **条目效果行**：bot 引用次数、客服（agent）引用次数、文章被踩数、客服 flag 数、解决率、向量化状态；引用数低于样本量下限（建议值 10 次 · 终稿前确认）显示「样本积累中」，不显示误导性数值。
...
- **候选**：类型（**新增 / 修订 / 合并** 三类）、标题、来源会话数与渠道分布、频次（次/周）、语义查重值、缺口判定（未覆盖 / 已覆盖但答不好 / 重复建条）、AI 摘要、建议动作。
- **三重准入（沉淀判断，全过才成为候选）**：①频次达阈值（建议值 ≥10 次/周 · 终稿前确认）；②语义查重——与既有条目相似度 **≥0.85 不新建、挂为该条目修订建议**（防重复建条分流 bot 引用）；③缺口判定（对照场景覆盖与既有条目）。合并类候选来自本台查重发现（两条目相似度 ≥0.85 且互相分流），合并后另一条在 Zendesk 端归档并重定向。
...
- **异常原则（写死）**：任何失败态（同步失败/翻译失败/向量失败/批次失败）都**如实标注 + 保留上次可用内容 + 提供重试路径**，不静默吞错、不带病上线。
...
2. **统一过审**：全部新条目/修订/挖掘候选/优化建议一律经审核中心，人工审核通过并过发布门禁后才生效同步——**无人审不生效，任何角色无绕过审核的发布路径**（含知识管理员无发布权限、审核员四眼原则）。
...
> **四眼原则（硬约束）**：知识审核员也可编辑与提交，但自己提交的条目须**另一名审核员**通过，不可自审自发。矩阵仅系统管理员可修改，每次修改写审计日志。
...
| 结构树 | 两级树（目录 → 章节 → 条目），章节行带 Zendesk Section 映射标识（如 Sec 5101） | 可折叠展开 | 点条目进入条目工作台 | 无 |
| 结构映射说明 | 说明条 | 常驻：「映射 Zendesk：知识库 → Help Center brand、目录 → Category、章节 → Section、条目 → Article、条目内锚点 → 文章内 anchor」 | 只读 | 无 |
| 条目列表 | 表格：标识/标题/路径/类型/可见性/版本/状态/英文状态/同步状态/解决率/复核到期/向量化状态/更新时间 | 按更新时间降序；低解决率标红；复核超期标红（如「复核已到期 12 天」）、临期标黄 | 点行进入条目工作台 | 无 |
...
**状态流转条**：草稿 → 编辑中 → 待审核 → 审核中 → 审核通过 → 已发布（异常支线：已驳回 / 已下线），当前态高亮、已过态打勾；旁附最近操作人与时间。**操作按钮**：保存中文草稿 / 提交审核 / 发布并同步——按权限矩阵与当前状态禁用（如知识管理员的「发布并同步」恒禁用并提示「统一过审铁律：知识管理员不能直接发布，必须由知识审核员在审核中心通过后发布」）。**门禁提示条**：实时显示阻断原因（英文未确认 / 向量待重建）或「门禁通过：正文已保存、英文已确认、向量已就绪，可发布并同步」。

**四个页签**：

1. **知识正文**：富文本编辑器（标题层级/加粗/列表/表格/图片占位与图片说明·同步 Zendesk/超链接/引用块/附件/清除格式）+ 字段侧栏（所属知识库/目录章节/两级问题场景/标签·同步 Zendesk labels/可见性三档/适用型号/复核周期/负责人，字段对齐 §4.1）；**中英双语切换**：中文（权威源）/ 英文（同步 Zendesk）两段视图——英文视图为逐段中英对照（段落级），人工修订过的段落带修订标注（如「人工把 buyer's remorse 改为 change of mind，更贴近北美用户表述」），**内部段落显示「内部段落不翻译、不同步到对外帮助中心」**；英文操作：生成/重新翻译、标记已确认、同步到 Zendesk（按钮随英文状态机与权限启停）。只读角色（AI 运营）编辑器锁定并明示「只读模式」。**向量状态面板**：状态（未向量化/向量化中/已向量化/待重建/向量失败）、分片信息、最后向量化时间、用途说明（挖掘查重·缺口聚类·发布门禁代理评测）、重建按钮；待重建时提示「内容已变更，向量落后于正文——发布前必须重建，否则查重与代理评测跑的是旧内容」。
...
3. **效果指标**：当前版本五卡（调用次数/命中率/解决率·近似归因标注/采纳率/向量状态）+ 各版本解决率对比条形 + 条目级反馈信号列表（文章被踩/bot 未解决转人工/客服 flag/文章投票，含原文摘录与次数）。
...
| 三重准入说明条 | 说明 | 常驻：「沉淀判断三重准入：①频次达阈值 ②语义查重 ③缺口判定」 | 只读 | 无 |
| 候选卡 | 卡片（类型徽章：新增/修订/合并；标题；来源会话与渠道分布；频次；查重值；缺口判定；AI 摘要；准入结论） | 查重 ≥0.85 高亮并注明「不新建，挂修订」；合并类注明「合并后另一条 Zendesk 归档并重定向」 | 主操作按类型：起草并提交审核 / 挂为既有条目修订建议 / 发起合并（进审核）；次操作：丢弃（留痕，同主题再达阈值会重新出现） | 无提交权限角色降级为「提交优化建议」；AI 服务不可用时批次标失败、人工流程照常 |
...
| 审核详情 | 正文全文 + **变更前后对照表**（字段/变更前/变更后，如 退款时限：签收日起算 → 跨境订单按清关日起算） | 展示来源与频次证据 | — | — |
| 审核操作 | 通过 / 驳回 双按钮 + 门禁说明（「通过后自动跑发布门禁：静态检查 + 英文校验状态 + 代理评测，全过才入同步队列」） | 仅审核员可操作；其余角色「只能查看，不能通过或驳回」 | 通过→发布门禁→同步队列；**驳回→理由必填弹窗**（空理由不可提交，提示「驳回理由必填——审核意见会回传给提交人并留痕」），确认后条目回提交人草稿箱、状态「已驳回」 | 非审核员禁用；**四眼原则**：提交人=当前审核员时通过按钮禁用并说明 |
| 审核历史 | 时间线（驳回/重新提交/发布记录及理由，示例：李骁驳回「计费周期与财务口径不符，请附财务确认截图」→ 王雯重提「已补充财务确认截图与跨月退订示例」） | 每条目可查 | — | — |
| 发布门禁结果 | 四查清单：①格式与字段完整性（标题/章节/可见性/锚点/标签已填）②敏感信息与内部口径（内部段落已标记，不会进对外文章）③英文版本状态（未确认则同步将被阻断）④**代理评测**（本台向量检索召回验证，如实标注「代理指标不等于 Zendesk AI 真实表现」） | 通过项打勾、警示项标黄 | 任一硬项不过→不入同步队列并展示原因 | 无 |
#### 视图⑥ 同步中心 · Zendesk
...
1. **知识库效果**：场景覆盖条（一级场景 × 条目数 × 覆盖率，对照工单场景分布；低覆盖标红，示例：配件兼容 19%）+ **条目效果列表**（逐条目：bot 引用 / agent 引用 / 被踩 / 客服 flag / 解决率 / 版本 / 归属章节 / 向量化状态；低解决率标红加粗浮顶；样本不足显示「样本积累中」）；点条目行直达条目工作台效果页签。口径唯一落点 §8.2。
...
2. **角色管理**：四角色卡（人数/允许清单/禁止清单/角色定位说明——知识管理员「知识生产主力，所有产出必须过审，无直接发布路径」；知识审核员「质量与发布唯一执行者，四眼原则硬约束」；AI 运营「只读 + 建议角色，把数据洞察变成审核队列里的建议」；系统管理员「职责分离：管人不管内容」）。
...
| 向量状态枚举 | 「未向量化」「向量化中」「已向量化」「待重建」「向量失败」 |
...
| 门禁通过提示 | 「门禁通过：正文已保存、英文已确认、向量已就绪，可发布并同步。」 |
...
| 三重准入说明 | 「沉淀判断三重准入：①频次达阈值 ②语义查重 ③缺口判定」；查重超线「查重 ≥ 0.85 → 不新建，挂修订」 |
...
| 代理评测标注 | 「代理指标不等于 Zendesk AI 真实表现，如实标注。」 |
| 样本不足 | 「样本积累中」 |
...
- **向量失败/待重建**：失败条目不参与查重与代理评测（会漏判重复建条，界面明示）；发布前待重建条目须先重建。
...
- 挖掘抓取节奏 = 每日（建议值 · 终稿前确认）；频次准入阈值 = ≥10 次/周（建议值）；语义查重挂修订线 = ≥0.85（v3 定稿）。
...
flowchart LR
    ...
    A1 --> R["审核中心（统一过审）<br/>四来源 · 驳回理由必填 · 四眼原则"]
    A2 --> R
    A3 --> R
    R -->|"通过"| G["发布门禁四查<br/>①格式字段 ②内部段落标记<br/>③英文校验状态 ④代理评测"]
    R -.->|"驳回（理由留痕）"| A1
...
stateDiagram-v2
    ...
    待审核 --> 审核中 : 审核员受理
    审核中 --> 审核通过 : 通过（四眼原则：不可自审）
    审核中 --> 已驳回 : 驳回（理由必填留痕）
    已驳回 --> 待审核 : 按意见修改后重新提交
    审核通过 --> 已发布 : 发布门禁四查全过 → 入同步队列
...
| 向量失败 / 待重建 | 向量服务异常 / 内容变更 | 状态徽章 + 「不参与查重与代理评测」提示 | 手动/自动重建；发布前强制重建 |
...
- **代理评测≠真实表现**：发布门禁代理评测（本台向量检索）结果固定标注「代理指标不等于 Zendesk AI 真实表现」，不得用代理指标对外宣称解决率。
...
| ZR-4 | **RBAC 权限失配** | 权限配置过松（绕过审核发布）或过紧（无人能发布），口径事故或流程死锁 | 权限矩阵产品化定稿（§5.10，10 项×4 角色）；四眼原则硬约束；矩阵修改仅系统管理员且留痕；AC-P-22/23 验收 |
...
- **RBAC 角色到人**：知识管理员/知识审核员/AI 运营按拍板 2 兼任安排映射到人（名单后补机制延续）；审核员至少 2 人（四眼原则的最低人数前提）；系统管理员 1 人。
...
- 中台前端 = Web 单页应用（React 方向沿用既有技术栈决策）；后端 = Node 服务沿用既有选型；数据库 = Postgres + pgvector（向量能力降级为内部工具：挖掘查重/缺口聚类/代理评测——08-04-2026 15:55 拍板③延续）；LLM = Claude 方向（挖掘聚类/翻译/整理建议）。
- 原自研对话引擎（RN 对话页/SSE 流式/意图分类管线）冻结不再演进；可复用资产=知识条目存储与向量检索模块。
...
| AC-P-02 | ②知识库总览 | P0 | 打开知识库总览并切换三个知识库 | 多库可切换（含「客服话术库·仅内部」带不对外说明）；结构树按 目录→章节→条目 两级可折叠导航，章节行带 Zendesk Section 映射标识；条目行可见 状态/可见性/版本/英文状态/同步状态/解决率/复核到期/向量化状态 |
| AC-P-03 | ②知识库总览 | P1 | 使用搜索与组合筛选（场景/状态/可见性/复核到期） | 结果即时过滤且标注归属路径；「已到期/30 天内到期」筛选可用，超期条目标红 |
| AC-P-04 | ③条目工作台 | P0 | 知识管理员编辑「退款政策」正文并保存中文草稿 | 富文本与字段侧栏（库/章节/两级场景/标签/可见性/型号/复核周期/负责人）可编辑保存；保存后英文状态自动置「待重新校验」、该条目同步被阻断且原因可见；向量状态置「待重建」 |
...
| AC-P-10 | ⑤审核中心 | P0 | 审核员打开待审队列 | 四来源徽章（AI 挖掘/人工录入/版本修订/反馈修订）可辨；单条可见正文与变更前后对照；通过后自动进入发布门禁；待审计数在导航实时可见 |
| AC-P-11 | ⑤驳回理由必填 | P0 | 审核员驳回一条待审条目且理由留空 | 空理由无法提交（必填校验提示可见）；填写后条目回提交人草稿箱、状态「已驳回」、理由写入审核历史并在提交人工作台可见；重新提交后审核历史含完整往返记录 |
| AC-P-12 | 发布门禁 | P0 | 一条英文状态为「待人工校验」的条目通过审核 | 门禁四查展示（格式字段/内部段落标记/英文状态/代理评测）；英文未确认→不入同步队列、阻断原因可见；代理评测结果附「代理指标不等于 Zendesk AI 真实表现」标注 |
...
| AC-P-17 | ⑦数据看板·效果 | P0 | 打开知识库效果页签 | 场景覆盖条（对照工单场景分布，低覆盖标红）与条目效果列表（bot 引用/agent 引用/被踩/flag/解决率/版本/向量状态）可见；低解决率条目标红浮顶；引用数低于样本下限显示「样本积累中」（承接原 AC-E-15 语义）；点行直达条目效果页签 |
...
| AC-P-23 | 四眼原则 | P0 | 知识审核员审核自己提交的条目 | 通过操作不可执行（禁用并说明四眼原则）；须由另一名审核员处理 |
...
| 四眼原则 | 审核员自己提交的条目须另一名审核员通过，不可自审自发 |
...
| 代理评测 | 发布门禁中用本台向量检索做的召回验证；固定标注不等于 Zendesk AI 真实表现 |
| 向量化状态 | 条目向量索引状态；向量只服务本台内部（查重/聚类/代理评测），与 Zendesk AI 索引无关 |
...
| AC | 80 注册（55 retired + 25 现役 AC-P-01…25） | §10.1 / §10.5 | 现役 AC 全部指向 §5.10 视图规格与 §4 schema；铁律覆盖核对：权限矩阵生效=AC-P-22、统一过审=AC-P-05、驳回理由必填=AC-P-11、同步阻断=AC-P-12/13、版本回滚=AC-P-06、drift 报警=AC-P-15、翻译人工校验后才同步=AC-P-16、四眼原则=AC-P-23 |
