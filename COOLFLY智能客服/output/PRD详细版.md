...
| Zendesk API 凭据（Guide 文章读写、工单/会话读取、Knowledge 面板） | **ready（08-05-2026 用户提供真实 OAuth client_credentials，只读联调通过：取到 6 个真实 Section、9 篇存量文章、locale en-us+zh-cn）** | **内部技术团队** | 同步中心、AI 对话挖掘、drift 检测均依赖；限流边界见 §7.2；写操作默认关闭（安全闸），待章节→Section 映射校准后经用户确认再开启 | 无 API 则同步退化为人工粘贴，中台价值大减 | MVP |
| **中台内部账号体系（新前置）** | 待建 | 内部技术团队 | RBAC 四角色需**真实账号**（v3 拍板）；方案「建议值 邮箱+密码邀请制、首次登录强制改密 · 终稿前确认」 | 无账号体系则权限矩阵与审计日志失去主体，不可上线 | MVP |
| LLM API（挖掘聚类起草、AI 翻译、整理建议） | **ready（08-05-2026 用户提供通义千问真实凭据，联调通过：翻译/摘要/主题提炼/查重判定/整理建议五项能力全部真实验证）** | 内部技术团队 | **供应商 08-05-2026 由 Anthropic Claude 改为通义千问**；DPA 且输入不用于训练仍为选型硬条件（§9.7），供应商换了硬条件不变、证据仍未就绪 | 无 LLM 则挖掘/翻译退化为纯人工，闭环价值大减 | MVP |
...
| 章节管理 | 结构操作（新建目录/章节、重命名、调整层级、删除） | 有「管理目录与章节」权限可见 | 变更即时反映并留痕；**实时同步 Zendesk**（目录→Category、章节→Section；08-05-2026 拍板：结构只在本台维护，Zendesk 侧不再手工维护） | 无权限禁用；含条目的章节不可直接删除（先移空）；Zendesk 端仍有文章/Section 时拒绝删除 |
...
| 元素 | 类型 | 默认态 | 操作后 | 禁用条件 |
|------|------|--------|--------|---------|
| 待审队列 | 列表（来源徽章：AI 挖掘·新增 / 人工录入 / 版本修订 / 反馈修订；标题/频次/归属章节与建议可见性/版本说明/提交人/提交时间） | 待审计数在导航常驻可见；按提交时间排序、AI 挖掘候选附频次 | 点条打开审详情——**审核员打开即认领**（08-05-2026 补齐：条目状态 待审核→审核中，其他审核员可见谁在审，避免重复劳动；已认领的重复打开不报错） | 无 |
...
| Zendesk API | 同步推送、会话/工单拉取（挖掘）、drift 比对 | **ready（08-05-2026 真实凭据只读联调通过）** | **内部技术团队** | 429 限流→批次失败如实标注次日重拉、同步退避重试（§7.2，v3 批次示例）；凭据失效→告警指向 owner；写操作安全闸默认关闭，须显式开启 |
| LLM API（挖掘/翻译/建议，08-05-2026 供应商改通义千问） | 会话主题提炼、聚类起草、中→英翻译、条目摘要、语义查重判定、整理建议 | **ready（真实凭据联调通过）** | 内部技术团队 | DPA 硬条件（§9.7，供应商换了硬条件不变，证据仍未就绪）；不可用时人工路径照常（§7.1） |
...
- 中台前端 = Web 单页应用（React 方向沿用既有技术栈决策）；后端 = Node 服务沿用既有选型；数据库 = Postgres（**08-05-2026 起不含 pgvector**——向量能力整体删除，挖掘查重改由 LLM 依条目 AI 摘要语义判定）；LLM = **通义千问方向**（08-05-2026 二轮由 Claude 改，用户拍板；挖掘聚类与查重判定/翻译/条目摘要/整理建议/会话主题提炼）。
