...
| **阶段二及以后（后置，未排期、未细分，照抄已确认清单）** | 差异化与全渠道（本 PRD 不展开） | F07 完整多语言；设备状态诊断 + 自动解决方案（差异化王牌，依赖方案商接口争取）；邮箱/社媒/官网全渠道接入（长期愿景：一个 agent 服务全渠道）；Zendesk 客服工作台接入；方案商设备日志接口调用；**语音/无屏交互渠道**（App 内语音输入与答案朗读、智能音箱形态；企业家 08-03-2026 拍板登记，接口预留 = §3.7 引擎渠道无关 + 埋点 channel 字段 + §4.1 知识条目渠道中立，本期零额外开发）；**AI 生成型知识库**（F05 演进：AI 从多渠道客诉数据〔App 会话/应用商店评分/客服邮件〕自动聚类并起草知识条目，人工审核后发布；禁止无人审全自动生效；接口预留 = 知识条目 schema、channel 字段、F05 回流管道与评测集回归，本期零额外开发） | —（后置项不在本 PRD 承诺范围内，仅作演进方向登记） |
...
> 阶段二演进方向（已登记 §0 后置清单，本期不实现）：本回流管道升级为「AI 起草 + 人工审核」的生成型知识库——AI 从回流清单与多渠道客诉数据（应用商店评分/客服邮件，依 channel 字段接入）自动聚类并起草条目草稿，运营岗职责从"写条目"变为"审核发布"；条目发布前仍须过 §4.3 评测集回归，禁止无人审全自动生效。本期接口预留即 §4.1 条目 schema、§4.4 聚合键与本节回流管道，零额外开发。
...
| AC-E | 12（AC-E-01–12） | §10.2 | AC-E-02↔§3.3 连续计数；AC-E-03↔§3.1 关键词规则（清单 v1 §12.7，R5-T1）；AC-E-06↔§5.2 超范围专属文案（R5-S3）；AC-E-07↔§5.4 缓冲系数定数；AC-E-09↔§5.4 摘要降级；AC-E-11↔§5.9 降级行为，均闭合 |
