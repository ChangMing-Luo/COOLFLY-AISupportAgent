...
### US-F09-06 · 单知识库结构树映射 Zendesk（revision 5 增补，PRD §2.4；08-05-2026 单库收敛）

- Role: 知识管理员
- Goal: 在**单知识库**（08-05-2026 拍板：系统只有一个知识库，界面无库切换、无库名概念）的「目录→章节→条目」结构树中组织知识，结构映射 Zendesk（目录→Category、章节→Section）
...
### REQ-F09-12 · 知识库总览（视图②：单库三级结构树 / Zendesk 结构映射 / 复核到期；08-05-2026 修订）
...
```text
THE SYSTEM SHALL 提供**单知识库**（08-05-2026 拍板：系统只有一个知识库，界面无库切换、无库名概念）的**三级折叠树**（目录 → 章节 → 条目，逐级缩进且逐级可折叠；目录行带文件夹图标与条目计数、条目行带文档图标与省略号截断的标题；树内仅呈现已发布条目并以「仅已发布」徽章标注；结构操作按钮（新建目录 / 新建章节 / 调整层级）收进树卡顶部工具条，重命名/删除随悬停或选中显示、不常驻挤压行宽；调整层级 = 改章节所属目录，结构深度恒为 2），章节行**不**常驻 Zendesk Section 映射标识——结构映射规则（目录→Category、章节→Section、条目→Article、条目内锚点→文章内 anchor；规则唯一落点 §4.1，本视图引用不复制）与结构操作约束统一收进卡头「?」说明弹窗（提示信息不占主内容容器，信息密度优先）。
条目列表 SHALL 含 标识/标题/路径/类型/可见性/版本/状态/英文状态/同步状态/解决率/复核到期/更新时间，按更新时间降序；路径列按「目录 / 章节」显示归属（不含库名）；低解决率标红；复核超期标红（含超期天数）、临期标黄；搜索与组合筛选（场景/状态/可见性/复核到期：全部/已到期/30 天内到期）即时过滤且结果标注归属路径；问题场景筛选选项 SHALL 来自元数据场景字典（REQ-F09-20，一级场景列表），不随条目数据派生。
...
- Stage: MVP
- Revision: 9（08-05-2026 元数据字典化：字段侧栏三个元数据控件改造——问题场景改为**字典化联动下拉**（选项来自 REQ-F09-20 场景字典，废止「跟目录走系统派生」拍板）、标签改**可视化 chips 多选器**（数据结构 labels 不变，交互不再用逗号分隔文本）、适用型号改**按条目类型条件显隐**（操作流程型/FAQ 型显示、FAQ 政策型/内部口径不显示，选项来自型号字典，空=全型号）。历史 rev8：补齐**条目下线**入口——`published → offline` 此前虽在状态机内合法却无任何 API/按钮可达；下线 = Zendesk 归档 + 重定向不物理删除，版本与指标保留，且 `offline → editing` 保证可重新上架不成单向陷阱。历史 rev7：正文编辑面改为**双语富文本编辑器**——左中文右英文同屏逐行对齐、富文本行内标记落 `paragraph.html`、「小标题」与「内部段落」两个勾选框取消（分别由 H2/H3 与工具条块级标记按钮承担）；标题单独成卡做中英对照。历史 rev6：**向量状态面板整体删除**，替换为 **AI 摘要面板**——摘要在发布时由 LLM 生成、可人工校正且校正后不被覆盖，只服务本台内部语义查重；效果指标由五卡收敛为四卡（去「向量状态」卡））
- Status: active
- Behavior (EARS):

```text
THE SYSTEM SHALL 提供状态流转条（草稿→编辑中→待审核→审核中→审核通过→已发布；异常支线：已驳回/已下线；当前态高亮、已过态打勾）与四页签：知识正文（**双语富文本编辑器**：左中文右英文同屏逐行对齐，一行=一段落；左格富文本支持 加粗/斜体/删除线/H2/H3/有序与无序列表/引用块/图片，行内标记存 `paragraph.html`、纯文本存 `paragraph.text`、`heading` 由块类型派生；工具条含「标为内部段落」块级标记按钮，取代原「内部段落」「小标题」两个勾选框；标题单独成卡做中英对照 + 字段侧栏对齐 §4.1：目录章节/两级问题场景（**一级/二级联动下拉，选项实时来自元数据场景字典 REQ-F09-20——禁止前端硬编码场景常量**）/标签·同步 Zendesk labels（**可视化 chips 多选器**：搜索全库既有标签建议 + 回车新建 + 已选显示为可删除标签卡 + 上限「建议值 10 条」提示——不接受逗号分隔自由文本录入）/可见性三档/适用型号（**按条目类型条件显隐**：操作流程型/FAQ 型显示型号多选〔选项来自型号字典，空值=全型号〕、FAQ 政策型/内部口径**不显示**该字段）/复核周期/负责人）、版本与回滚、效果指标、操作日志（条目级全量留痕，schema §4.5）；操作按钮（保存中文草稿/提交审核/发布并同步）按权限矩阵与当前状态禁用并说明（统一过审铁律提示见 REQ-F09-04）；只读角色（AI 运营）编辑器锁定并明示「只读模式」。
...
### REQ-F09-20 · 元数据管理（视图⑪：场景字典 / 型号字典 / 标签聚合；revision 9 增补 · 08-05-2026 元数据字典化）

- Story: US-F09-06（元数据字典化承载）
- Anchor: FR-F09（PRD §5.10 视图⑪；§4.1 元数据 schema）
- Stage: MVP
- Revision: 9
- Status: active
- Behavior (EARS):
...
```text
THE SYSTEM SHALL 提供**元数据管理**视图⑪（新菜单，08-05-2026 元数据字典化拍板——**废止 PRD v2.1 §13「问题一二级分类跟着目录走、系统派生只读」**，场景改为独立可维护字典），三个区块：
①**场景字典**：两级场景（一级/二级）增删改与排序（深度恒 2，二级必须挂一级）；被条目引用的场景禁止删除（提示引用条目数，先改引用再删）；删除/改名写审计日志；条目「问题场景」选项 SHALL 全部来自本字典（前端禁止硬编码场景常量），知识库总览筛选与数据看板场景统计同样从字典取数。
②**型号字典**：型号增删改（列表+排序）；被条目引用的型号禁止删除；型号只服务本台条目元数据（Zendesk 无对应对象，不同步）；条目「适用型号」选项来自本字典。
③**标签聚合**：只读展示全库标签池（标签名 + 引用条目数），供条目编辑器搜索建议与治理决策；标签治理（同义归并/重命名/90 天零命中冷却清理）登记为后续版本，本轮不做。
写操作（场景/型号增删改）SHALL 仅「管理目录与章节」权限角色（structure.manage）可执行；读取登录即可。
```
...
### AC-F09-15 · 知识库总览单库三级结构树可导航（Revision 7 · 08-05-2026 单库收敛）
...
```text
WHEN 打开知识库总览
THE SYSTEM SHALL 使单知识库可用（界面无库切换、无库名概念）；结构树按 目录→章节→条目 **三级**可折叠导航（逐级缩进、树内仅呈现已发布条目并带「仅已发布」徽章、结构操作在顶部工具条与悬停行内），章节行不显示 Zendesk Section 映射标识（映射规则与操作约束收进卡头「?」说明弹窗）；条目行可见 状态/可见性/版本/英文状态/同步状态/解决率/复核到期。
```
...
### AC-F09-40 · 元数据字典维护与引用约束（revision 9 增补）

- PRD 对照 ID: AC-P-27（PRD §10.1）
- Parent: REQ-F09-20
- Priority: P0
- EARS:

```text
WHEN 有「管理目录与章节」权限的角色在元数据管理视图新建/重命名/删除场景或型号
THE SYSTEM SHALL 使场景两级字典（深度恒 2，二级必须挂一级）与型号字典可增删改并排序；被条目引用的场景/型号删除被拦截并提示引用条目数（先改引用再删）；删除与改名写审计日志；条目编辑器与总览筛选的问题场景选项来自字典（前端禁止硬编码场景常量）。
```

### AC-F09-41 · 条目元数据录入交互（场景字典联动 / 标签 chips / 型号条件显隐）（revision 9 增补）

- PRD 对照 ID: AC-P-28（PRD §10.1）
- Parent: REQ-F09-13
- Related-REQ: REQ-F09-20
- Priority: P0
- EARS:

```text
WHEN 知识管理员在条目工作台录入元数据
THE SYSTEM SHALL 使问题场景为一级/二级联动下拉（选项实时来自场景字典，非硬编码）；标签为可视化 chips 多选器（搜索既有标签建议、回车新建、已选显示为可删除标签卡、超上限提示——不接受逗号分隔自由文本）；适用型号按条目类型条件显隐（操作流程型/FAQ 型显示型号多选且选项来自型号字典、FAQ 政策型/内部口径不显示该字段），型号为空即「全型号」。
```
...
## 统计（revision 9 · 08-05-2026 元数据字典化）

- 稳定对象总计：**193**（US×36 + REQ×57 + AC×95 + NFR×5）；其中 **active 58 / retired 135**（retired 保留原 ID 与 EARS 正文为契约史料，永不复用）。revision 9 新增 REQ-F09-20 与 AC-F09-40/41，无对象退役——「问题场景跟目录走」为行为废止（PRD v2.1 §13 该条随元数据字典化废止），承载行为修订于 REQ-F09-12/13 原地续用。
- US：36 注册 = 24 retired（F01–F08×20 含契约层增补 US-F07-01；F09 五区版旧×4）+ 12 active（US-F09-05…16，revision 5 随 PRD §2.4 增补；US-F09-06 于 revision 9 修订为单库+场景字典化语义）。
- REQ：57 注册 = 43 retired（F01–F08×37；F09 旧×6：REQ-F09-01/03/05/06/09/10）+ 14 active（修订续用×4：REQ-F09-02/04/07/08 + 新增×10：REQ-F09-11…20；其中 REQ-F09-12/13 于 revision 9 原地修订：场景筛选与编辑器元数据控件字典化、型号按条目类型条件显隐）；active REQ 全部 Anchor=FR-F09、Stage=MVP。
- AC：95 注册 = 65 retired（F01–F08×54；F09 旧×11：AC-F09-01…05/07/08/09/11/12/13）+ 30 active（契约层增补续用×2：AC-F09-06/10 + 新增×28：AC-F09-14…41 ↔ PRD AC-P-01…28）；每条 AC 唯一 Parent，14 个 active REQ 全部由 active AC 覆盖（retired REQ 由 retired AC 覆盖，史料闭合）。
...
- 铁律覆盖核对（对齐 PRD §13.1）：权限矩阵生效=AC-F09-35、统一过审=AC-F09-18、驳回理由必填=AC-F09-24、同步阻断=AC-F09-25/26、版本回滚=AC-F09-19、drift 报警=AC-F09-28、翻译人工校验后才同步=AC-F09-29、审核自审留痕=AC-F09-36（原四眼原则已取消）、迁移无绕审=AC-F09-38、元数据字典维护与引用约束=AC-F09-40、元数据录入交互=AC-F09-41。
