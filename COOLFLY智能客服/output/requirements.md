...
### revision 7 · source_ids_changed（08-05-2026 二轮：双语富文本编辑器改版；追记，原会话未及时登记变更日志）

- **变更依据**：用户实操后反馈「新建条目沿用旧数据」「审核中心两栏不对齐」两个真 Bug，随后要求正文编辑面改版——左中文右英文同屏逐行对齐（取代原「中/英」页签切换），取消「小标题」「内部段落」两个勾选框（分别由 H2/H3 与工具条块级标记按钮承担），标题单独成卡做中英对照；合并协作者 PR#1（英文标题 en_title 翻译工作流）。
- **修订续用（Revision→7，ID 与 Anchor 不变）**：REQ-F09-13（正文编辑面整体改版，见对象内 EARS 全文）。
- **retire**：无。
- **补记**：本条变更日志在原会话中遗漏，本轮（revision 9）一并追加，object-level `Revision: 7/8` 标注与本节同步，此前仅对象内文本更新、顶层 revision 计数与变更日志缺失属文档驱动纪律的执行漏洞，本轮修正。

### revision 8 · source_ids_changed（08-05-2026 二轮：条目下线入口 + 一键批量导入入口；追记，同上）

- **变更依据**：用户问「收纳好的条目怎么删」，AI 扫描发现 `published→offline` 在状态机内合法却无任何入口可达，用户拍板补齐（非物理删除，Zendesk 端归档+重定向）；用户随后追加「一键导入」需求，AI 发现后端批量导入能力已具备（RULE-03 已覆盖）但缺前端入口，遂新建导入面板。
- **修订续用（Revision→8，ID 与 Anchor 不变）**：REQ-F09-13（新增下线 EARS：仅已发布可下线、仅审核员、二次确认、Zendesk 归档非删除、可重新编辑上架）。
- **新增稳定对象（Revision 起始=8）**：无新增 REQ/AC——下线复用 REQ-F09-13/REQ-F09-16（同步任务）既有结构；一键导入复用 REQ-F09-02（批量导入）既有入口，本轮（revision 9）才把 AI 整理建议正式写入 REQ-F09-02 EARS。
- **retire**：无。

### revision 9 · source_ids_changed（08-05-2026 三轮：LLM 供应商由 Anthropic Claude 改通义千问 + 八项功能缺口修复 + 三道安全闸）

- **变更依据**：用户质疑「翻译怎么实现的，为什么没问我要 SDK」，AI 承认此前 `ANTHROPIC_API_KEY` 从未配置、全程跑本地词表桩（非真实翻译），要求全量扫描；扫描出八项缺口后用户拍板供应商改通义千问并提供真实凭据（含生产 Zendesk OAuth）。
- **八项缺口**：①「审核中」状态不可达 ②翻译连续失败告警未落地（`TUNABLES.translateFailAlert` 定义了但全仓零引用）③摘要生成失败前端不标注（`zhCN.summary.generateFailed` 定稿但零渲染点）④复核到期无主动扫描 ⑤挖掘主题写死（`deriveTopics` 只取会话计数，恒定输出 3 条演示样例）⑥AI 整理建议零实现（`suggestLabels` 定义了但全仓零调用）⑦信号采集零实现（五张信号表只在种子脚本被写）⑧LLM 选型偏差未登记（技术方案原选 Vercel AI SDK，实现走原生 fetch）。
...
| Zendesk API（同步推送 / 会话与工单拉取（挖掘）/ drift 比对，PRD §9.2/§7.2） | 内部技术团队 | 内部技术团队 | deployment-secret | system | **ready（08-05-2026 用户提供真实 OAuth client_credentials，只读联调通过：取到 6 个真实 Section、9 篇存量文章）**；写操作默认关闭（安全闸，需显式开启）；429 限流→批次失败如实标注次日重拉、同步退避重试、初始迁移分批推送避开峰值（节奏「建议值 · 终稿前确认」）；凭据失效→告警指向 owner | MVP | REQ-F09-14、REQ-F09-16 |
| LLM API（08-05-2026 供应商改通义千问；会话主题提炼 / 挖掘聚类起草 / 中→英翻译 / 条目摘要 / 语义查重判定 / 整理建议；硬条件=供应商提供 DPA 且输入不用于训练，PRD §9.2/§9.7） | 内部技术团队 | 内部技术团队 | deployment-secret | system | **ready（08-05-2026 用户提供真实凭据，五项能力联调通过）**；DPA 硬条件不变、证据仍未就绪；不可用时人工路径照常（AI 为增强不是依赖，§7.1）；翻译失败保留上次英文并阻断同步 | MVP | REQ-F09-02、REQ-F09-13、REQ-F09-14 |
...
| Zendesk API 凭据（Guide 文章读写 / 工单与会话读取 / Knowledge 面板） | ready | 内部技术团队 | **08-05-2026 真实凭据只读联调通过**；同步中心、AI 对话挖掘、drift 检测均依赖；限流边界 §7.2；写操作安全闸默认关闭 | 无 API→同步退化为人工粘贴、挖掘退化为人工整理（价值大减，不作为验收态） | MVP | REQ-F09-14、REQ-F09-16 |
...
| LLM API 选型与部署（08-05-2026 供应商改通义千问；DPA 硬条件） | ready | 内部技术团队 | **真实凭据联调通过**；DPA 证据上线前完成（PRD §9.7 上线前置，供应商换了硬条件不变） | 不可用→挖掘/翻译/建议退化为纯人工路径照常（闭环价值大减，AI 为增强不是依赖） | MVP | REQ-F09-02、REQ-F09-13、REQ-F09-14 |
...
### REQ-F09-02 · 录入与批量导入（统一进审核中心 + 飞书一次性迁移 + AI 整理建议；08-05-2026 三轮修订）

- Story: US-F09-08
- Anchor: FR-F09（PRD §5.10 视图⑤来源「批量导入」/异常场景「批量导入部分失败」；§9.5 初始迁移计划；§0.5 飞书导出前置）
- Stage: MVP
- Revision: 9（08-05-2026 三轮：新增 **AI 整理建议**——批量导入解析出条目后，运营可一键请求 LLM 依标题与正文给出 标签（≤5 个）/两级问题场景/章节归类建议，逐条预览可改可忽略，建议态不直接写入、须人工确认；LLM 不可用时标 `degraded` 并如实标注「AI 建议未生效」。原 REQ-F09-03 retired 时误将此归入 REQ-F09-14，实际实现挂在本 REQ，见 REQ-F09-03 补记。历史 rev5（D-10）：入库面从「知识库总页面+F01 检索库」换锚为「知识库总览+同步 Zendesk」；审核主体=知识审核员（RBAC）；批量导入统一进审核队列/逐条失败报告/一次性迁移不双向同步语义延续）
- Status: active
- Behavior (EARS):
...
WHEN 运营在导入预览请求 AI 整理建议 THE SYSTEM SHALL 逐条回填 标签/两级问题场景/章节归类 建议（LLM 生成，建议态）；运营可逐条修改或忽略，不影响导入主流程；IF LLM 不可用 THEN 该条标「AI 建议未生效」，回退为空白待人工填写，不阻塞导入。
...
- Revision: 4
- Status: retired
- Retirement note: retired（08-04-2026 D-10）：AI 整理建议（建议态）承接=REQ-F09-14（挖掘候选起草）与 PRD §9.5 批量导入 AI 整理辅助；「AI 为增强不是依赖」降级原则由 §5.1 异常原则延续。**08-05-2026 补记**：实现时发现「切条建议」未落地（v3 原型无对应交互，PRD §5.10 亦未描述切条界面，登记为范围缩减非缺陷）；**标签/两级场景/章节归类建议实际实现挂在批量导入面板（REQ-F09-02），不在挖掘候选流（REQ-F09-14）**——承接指向修正为 REQ-F09-02，原「承接=REQ-F09-14」仅对候选起草文案生成部分成立
- Behavior (EARS):
...
- Stage: MVP
- Revision: 9（08-05-2026 三轮：复核到期由「查询时被动计算」补齐为**主动扫描**——每日定时任务扫描到期与临期条目，写入信号事件供反馈回流与告警引用，非仅列表页临时算 dueLevel。历史 rev6：①结构树由「两级可折叠」修订为**三级折叠树**——目录/章节/条目逐级缩进、树内仅呈现已发布条目、结构操作收进顶部工具条与悬停行内操作，UX/UI 以 v3 原型为准；②条目列表删除「向量化状态」列）
- Status: active
- Behavior (EARS):
...
复核到期 SHALL 按条目复核周期（「建议值 180 天，可选 90/365 天 · 终稿前确认」）自发布起算：到期提醒（临期「建议值 30 天内 · 终稿前确认」标黄）、超期标红并可被「已到期」筛选命中；THE SYSTEM SHALL 每日主动扫描一次到期与临期条目并写入信号事件（08-05-2026 三轮补齐——不再只是列表页打开时的被动计算，扫描结果可供反馈回流引用与后续告警渠道消费）。
```
...
- Stage: MVP
- Revision: 8（08-05-2026 三轮：补齐**条目下线**入口——`published → offline` 此前虽在状态机内合法却无任何 API/按钮可达；下线 = Zendesk 归档 + 重定向不物理删除，版本与指标保留，且 `offline → editing` 保证可重新上架不成单向陷阱。历史 rev7：正文编辑面改为**双语富文本编辑器**——左中文右英文同屏逐行对齐、富文本行内标记落 `paragraph.html`、「小标题」与「内部段落」两个勾选框取消（分别由 H2/H3 与工具条块级标记按钮承担）；标题单独成卡做中英对照。历史 rev6：**向量状态面板整体删除**，替换为 **AI 摘要面板**——摘要在发布时由 LLM 生成、可人工校正且校正后不被覆盖，只服务本台内部语义查重；效果指标由五卡收敛为四卡（去「向量状态」卡））
- Status: active
- Behavior (EARS):

```text
THE SYSTEM SHALL 提供状态流转条（草稿→编辑中→待审核→审核中→审核通过→已发布；异常支线：已驳回/已下线；当前态高亮、已过态打勾）与四页签：知识正文（**双语富文本编辑器**：左中文右英文同屏逐行对齐，一行=一段落；左格富文本支持 加粗/斜体/删除线/H2/H3/有序与无序列表/引用块/图片，行内标记存 `paragraph.html`、纯文本存 `paragraph.text`、`heading` 由块类型派生；工具条含「标为内部段落」块级标记按钮，取代原「内部段落」「小标题」两个勾选框；标题单独成卡做中英对照 + 字段侧栏对齐 §4.1：所属知识库/目录章节/两级问题场景/标签·同步 Zendesk labels/可见性三档/适用型号/复核周期/负责人）、版本与回滚、效果指标、操作日志（条目级全量留痕，schema §4.5）；操作按钮（保存中文草稿/提交审核/发布并同步）按权限矩阵与当前状态禁用并说明（统一过审铁律提示见 REQ-F09-04）；只读角色（AI 运营）编辑器锁定并明示「只读模式」。
...
条目下线 SHALL 仅对「已发布」条目开放且仅知识审核员可执行（其余角色按钮禁用、接口 403）：经二次确认后状态置「已下线」并写「归档 + 重定向」同步任务，Zendesk 端文章置归档而**非物理删除**（帮助中心不留死链）；版本历史、各版效果指标、审计日志 SHALL 全部保留；下线条目 SHALL 仍可编辑并自动回「编辑中」，重新过审发布即再次上架。**本系统 SHALL NOT 提供条目物理删除能力。**
...
- Stage: MVP
- Revision: 9（08-05-2026 三轮：主题提炼来源修订为**真实会话正文**——LLM 依脱敏后的工单/聊天原文归纳合并主题与命中会话数，取代此前「仅取会话计数、主题固定 3 条演示样例」的实现捷径；本地降级模式按会话首句粗聚合，同样不使用固定样例。历史 rev6：三重准入②由「向量相似度查重」修订为 **LLM 语义查重**——两段式（字面粗筛 Top-5 → LLM 依条目 AI 摘要判定），判定理由随候选落库并在候选卡展示；LLM 不可用时如实降级标注）
- Status: active
- Behavior (EARS):

```text
THE SYSTEM SHALL 按「建议值 每日 · 终稿前确认」批次拉取昨日 Zendesk 会话（邮件工单 + 在线聊天，来源会话脱敏后才进中台，§4.6），批次列表展示 抓取日期/来源会话数分渠道（邮件工单 n · 在线聊天 m）/候选数/批次状态（完成/无新草稿/失败），按日期降序；空批次标「无新候选：均未达频次阈值，如实标注」、失败批次标原因（如 Zendesk API 429 限流）且次日照常拉取——不静默缺批次，不阻塞人工录入与审核。THE SYSTEM SHALL 用 LLM 依脱敏后的真实会话正文归纳主题（同一件事合并为一条、给出命中会话数），**不得使用固定演示样例冒充挖掘结果**；LLM 不可用时按会话首句粗聚合降级，界面不隐藏降级事实。
...
- Stage: MVP
- Revision: 9（08-05-2026 三轮：新增**认领机制**——审核员打开待审详情即 `pending_review → reviewing`（原「审核中」态在状态机内合法但从无入口可达，本轮补齐入口），幂等且已认领重复打开不报错。历史 rev6：①「变更前后对照表」修订为**两层**——摘要层只给大概摘要、详情层为 **git diff 视图**；②发布门禁由四查收敛为**三查**，第④查代理评测随向量能力删除退役；③审核操作去四眼原则约束）
- Status: active
- Behavior (EARS):

```text
THE SYSTEM SHALL 提供四来源统一待审队列（来源徽章：AI 挖掘·新增 / 人工录入 / 版本修订 / 反馈修订；批量导入条目亦经此队列，徽章「批量导入」）：按提交时间排序、AI 挖掘候选附频次，待审计数在一级导航常驻可见；WHEN 审核员打开待审详情 THE SYSTEM SHALL 使条目状态由「待审核」置「审核中」并记录认领人与时间（08-05-2026 三轮补齐）；已是「审核中」再次打开 SHALL NOT 报错；审核详情 SHALL 分**两层**展示变更：第一层「变更摘要」只给大概摘要——一行总览（如「正文 3 处改动 · 字段 2 处改动」；新建条目为「新建条目 · {n} 个段落」）+ 逐项一句话概述，不展开全文；第二层「具体变更」经「查看具体变更」进入，采用 **git diff 视图**——按段落切分对齐、删除行 `-` 红底 / 新增行 `+` 绿底 / 未变更上下文行灰字且默认折叠（前后各留 1 行，可展开全部），行首带变更前后行号槽，字段变更以同形态键值行呈现，内部段落带标记。另展示 正文全文 + 来源与频次证据 + 审核历史时间线（驳回/重提/发布记录及理由）。
...
### AC-F09-23 · 审核中心四来源队列与变更摘要+git diff 两层对照（Revision 9 · 08-05-2026 三轮补认领）
...
```text
WHEN 审核员打开待审队列
THE SYSTEM SHALL 使四来源徽章（AI 挖掘/人工录入/版本修订/反馈修订）可辨；单条先见**变更摘要**（一行总览 + 逐项一句话概述），点「查看具体变更」展开 **git diff 视图**（删除行 `-` 红底 / 新增行 `+` 绿底 / 上下文灰字可展开全部）；打开待审详情即认领（状态置「审核中」，08-05-2026 三轮补齐）；通过后自动进入发布门禁；待审计数在导航实时可见。
```
...
## 统计（revision 9 · 08-05-2026 三轮：千问+八项缺口修复）

- 稳定对象总计：**190**（US×36 + REQ×56 + AC×93 + NFR×5，与 revision 6 持平）；其中 **active 55 / retired 135**。revision 7/8/9 均无新增/新退役对象——双语编辑器、下线入口、一键导入、千问切换、八项缺口修复全部原地修订既有 REQ/AC（REQ-F09-02/12/13/14/15，AC-F09-23），不新建、不复用旧 ID。
- US：36 注册 = 24 retired（F01–F08×20 含契约层增补 US-F07-01；F09 五区版旧×4）+ 12 active（US-F09-05…16，revision 5 随 PRD §2.4 增补）。
- REQ：56 注册 = 43 retired（F01–F08×37；F09 旧×6：REQ-F09-01/03/05/06/09/10）+ 13 active（修订续用×4：REQ-F09-02/04/07/08 + 新增×9：REQ-F09-11…19；REQ-F09-04/07/12/13/14/15 于 revision 6 原地修订，REQ-F09-13 revision 7/8 续修，REQ-F09-02/12/14/15 于 revision 9 再修）；active REQ 全部 Anchor=FR-F09、Stage=MVP。
- AC：93 注册 = 65 retired（F01–F08×54；F09 旧×11：AC-F09-01…05/07/08/09/11/12/13）+ 28 active（契约层增补续用×2：AC-F09-06/10 + 新增×26：AC-F09-14…39 ↔ PRD AC-P-01…26；AC-F09-23 于 revision 9 原地修订补认领机制）；每条 AC 唯一 Parent，13 个 active REQ 全部由 active AC 覆盖（retired REQ 由 retired AC 覆盖，史料闭合）。
