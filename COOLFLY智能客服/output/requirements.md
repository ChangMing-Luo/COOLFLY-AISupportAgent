...
- revision: 5
- source_prd: PRD详细版.md（951 行，commit a91bee9 Zendesk 转向大重构：D-10 架构基准——C 端服务归 Zendesk 生态（Guide/AI bot/Support/Explore），自研范围=知识运营中台十视图五组（功能标准=v3 原型）；§13 一致性自检为 ID 权威）
- status: active
...
| RBAC 角色到人（拍板 2 兼任映射延续；审核员 ≥2 人=四眼原则最低人数前提，系统管理员 1 人） | committed | 企业家 | 上线前名单落实（PRD §9.4） | 审核员不足 2 人→四眼原则不可满足，发布动作暂缓上线（名单后补机制延续拍板 2） | MVP | REQ-F09-19、REQ-F09-04 |
...
- Role: 知识审核员
- Goal: 在审核中心统一处理四来源（AI 挖掘/人工录入/版本修订/反馈修订）的待审条目，看到变更前后对照与审核历史；驳回必须填写理由，理由回传提交人并留痕
- Value: 质量线守得住，驳回有据可查
- Stage: MVP
- Status: active

### US-F09-10 · 发布门禁四查与同步阻断可见（revision 5 增补，PRD §2.4）

- Role: 知识审核员
- Goal: 通过审核后系统自动跑发布门禁（格式字段完整性/内部段落标记/英文版本状态/代理评测），全过才入同步队列；同步失败自动重试后告警、可手动重试，英文未校验等原因的阻断可见原因
- Value: 不带病同步，阻断原因不用猜
- Stage: MVP
- Status: active
...
### REQ-F09-04 · 统一过审铁律（无人审不生效 + 四眼原则，横切全部入口；08-04-2026 D-10 修订续用）

- Story: US-F09-08
- Related-Stories: US-F09-09（审核执行面；契约 Story 取主故事）
- Anchor: FR-F09（PRD §5.10 三条权威原则之二 / RBAC 权限矩阵；§10 AC-P-05/23）
- Stage: MVP
- Revision: 5（D-10：从五区版「审核队列界面 REQ」修订为横切铁律——覆盖面从"人工+AI 录入"扩展为全部新条目/修订/挖掘候选/反馈建议/drift 拉回；生效面从 F01 检索库换锚为 Zendesk 同步；增补四眼原则硬约束；审核中心界面行为拆至 REQ-F09-15）
- Status: active
- Behavior (EARS):
...
四眼原则（硬约束）：知识审核员也可编辑与提交，但自己提交的条目须另一名审核员通过——提交人=当前审核员时通过按钮禁用并说明原因。
...
- Revision: 2
- Status: retired
- Retirement note: retired（08-04-2026 D-10）：评测集回归门禁被发布门禁四查取代（第④查=代理评测·本台向量检索召回验证，固定标注代理性质）——承接=REQ-F09-15；≤24h 生效链路随自研检索线废止，换为过审后入同步队列分钟级推送（PRD §5.10 视图⑥）
- Behavior (EARS):
...
- Stage: MVP
- Revision: 5（D-10：由「知识库数据+客服工作数据双子板（F06 同源自研埋点）」改组为十视图⑦三页签；数据源换锚 Zendesk 信号矩阵+中台自有数据；客服工作数据页签=不重建 Explore、仅口径说明与指向）
- Status: active
- Behavior (EARS):

```text
THE SYSTEM SHALL 在视图⑦「数据看板」提供三页签：①知识库效果——场景覆盖条（一级场景 × 条目数 × 覆盖率，对照工单场景分布，低覆盖标红）+ 条目效果列表（逐条目：bot 引用/agent 引用/被踩/客服 flag/解决率/版本/归属章节/向量化状态；低解决率标红加粗浮顶；点条目行直达条目工作台效果页签）；②知识缺口——缺口→动作列表（未覆盖→立新条 / 已覆盖但答不好→修订，动作按钮一键起草或挂修订进审核队列）+ 搜索无结果关键词列表（关键词/周次数/判定：无对应条目·有条目但命名不匹配·内部有口径未对外）；③客服工作数据——不重建 Zendesk Explore，展示口径说明与 Explore 指向 + 「去知识缺口」直达。
...
### REQ-F09-12 · 知识库总览（视图②：多库切换 / 两级结构树 / Zendesk 结构映射 / 复核到期；revision 5 增补）
...
```text
THE SYSTEM SHALL 提供多知识库切换卡（含「客服话术库·仅内部」带"不对外公开，仅挂客服 segment"说明）与两级结构树（目录 → 章节 → 条目，可折叠展开；章节行带 Zendesk Section 映射标识），并常驻结构映射说明（知识库→Help Center brand、目录→Category、章节→Section、条目→Article、条目内锚点→文章内 anchor；规则唯一落点 §4.1，本视图引用不复制）。
条目列表 SHALL 含 标识/标题/路径/类型/可见性/版本/状态/英文状态/同步状态/解决率/复核到期/向量化状态/更新时间，按更新时间降序；低解决率标红；复核超期标红（含超期天数）、临期标黄；搜索与组合筛选（场景/状态/可见性/复核到期：全部/已到期/30 天内到期）即时过滤且结果标注归属路径。
WHEN 有「管理目录与章节」权限的角色维护结构（新建目录/章节、重命名、调整层级）THE SYSTEM SHALL 使变更即时反映并留痕，同步中心据映射更新 Zendesk 结构；IF 章节含条目 THEN 不可直接删除（先移空，防孤儿条目）。
...
中英双语 SHALL 按翻译工作流铁律执行（唯一落点 PRD §5.10）：中文=唯一内容权威源；英文状态机 未生成→AI 翻译中→待人工校验→已确认→已同步（异常：翻译失败/待重新校验）；「已确认」前「同步到 Zendesk」禁用（人工校验后才同步）；中文变更即英文置「待重新校验」并阻断同步（联动提示文案见 §5.10）；英文允许人工独立修订表述但不得改变政策口径、修订留痕带标注；内部段落不翻译、不同步；翻译失败保留上一次英文、同步阻断、可重试，连续失败（「建议值 3 次 · 终稿前确认」）告警。
...
向量状态面板 SHALL 展示状态（未向量化/向量化中/已向量化/待重建/向量失败）、分片信息、最后向量化时间与用途说明（挖掘查重·缺口聚类·发布门禁代理评测，仅服务本台，与 Zendesk AI 索引无关）；内容变更置「待重建」并提示发布前必须重建；失败条目不参与查重与代理评测且界面明示。
...
候选 SHALL 经三重准入全过才产生：①频次达阈值（「建议值 ≥10 次/周 · 终稿前确认」）②语义查重——与既有条目相似度 ≥0.85 不新建、挂为该条目修订建议（防重复建条分流 bot 引用）③缺口判定（未覆盖/已覆盖但答不好/重复建条）；候选卡展示 类型徽章（新增/修订/合并）/标题/来源会话与渠道分布/频次/查重值/缺口判定/AI 摘要/准入结论；合并类注明"合并后另一条 Zendesk 归档并重定向"。
...
### REQ-F09-15 · 审核中心（视图⑤：四来源统一队列 / 驳回理由必填 / 发布门禁四查；revision 5 增补）
...
```text
THE SYSTEM SHALL 提供四来源统一待审队列（来源徽章：AI 挖掘·新增 / 人工录入 / 版本修订 / 反馈修订；批量导入条目亦经此队列，徽章「批量导入」）：按提交时间排序、AI 挖掘候选附频次，待审计数在一级导航常驻可见；审核详情展示 正文全文 + 变更前后对照表（字段/变更前/变更后）+ 来源与频次证据 + 审核历史时间线（驳回/重提/发布记录及理由）。
审核操作 SHALL 仅知识审核员可执行（其余角色「只能查看，不能通过或驳回」）：通过 → 自动跑发布门禁；驳回 → 理由必填弹窗（空理由不可提交，提示文案见 §5.10 文案定稿表），确认后条目回提交人草稿箱、状态「已驳回」、理由回传提交人并写入审核历史；四眼原则约束见 REQ-F09-04。
发布门禁四查（全过才入同步队列）：①格式与字段完整性（标题/章节/可见性/锚点/标签已填）②敏感信息与内部口径（内部段落已标记，不会进对外文章）③英文版本状态（未确认则同步将被阻断）④代理评测（本台向量检索召回验证，结果固定标注「代理指标不等于 Zendesk AI 真实表现」）；通过项打勾、警示项标黄；任一硬项不过 → 不入同步队列并展示原因。
```
...
结构映射表 SHALL 常驻可查（本台结构 ↔ Zendesk 结构实例视图，规则唯一落点 §4.1）；IF 结构映射失败（目标 Section 不存在/被删）THEN 同步失败并提示先在章节管理修复映射，不自动创建未经确认的 Zendesk 结构；IF 鉴权失效 THEN 失败原因显式区分「凭据失效」并告警指向 owner（内部技术团队）。
```
...
THE SYSTEM SHALL 使多库可切换（含「客服话术库·仅内部」带不对外说明）；结构树按 目录→章节→条目 两级可折叠导航，章节行带 Zendesk Section 映射标识；条目行可见 状态/可见性/版本/英文状态/同步状态/解决率/复核到期/向量化状态。
```
...
THE SYSTEM SHALL 使富文本与字段侧栏（库/章节/两级场景/标签/可见性/型号/复核周期/负责人）可编辑保存；保存后英文状态自动置「待重新校验」、该条目同步被阻断且原因可见；向量状态置「待重建」。
```
...
THE SYSTEM SHALL 使每条候选展示 频次/查重值/缺口判定 三项与类型（新增/修订/合并）；查重 ≥0.85 的候选不提供「立新条」而是「挂为修订建议」；处置=起草并提交审核 或 丢弃（留痕，同主题再达阈值重现）；无任何候选直接入库路径。
```

### AC-F09-23 · 审核中心四来源队列与前后对照
...
```text
WHEN 审核员打开待审队列
THE SYSTEM SHALL 使四来源徽章（AI 挖掘/人工录入/版本修订/反馈修订）可辨；单条可见正文与变更前后对照；通过后自动进入发布门禁；待审计数在导航实时可见。
```
...
THE SYSTEM SHALL 使门禁四查展示（格式字段/内部段落标记/英文状态/代理评测）；英文未确认→不入同步队列、阻断原因可见；代理评测结果附「代理指标不等于 Zendesk AI 真实表现」标注。
```
...
```text
WHEN 打开知识库效果页签
THE SYSTEM SHALL 使场景覆盖条（对照工单场景分布，低覆盖标红）与条目效果列表（bot 引用/agent 引用/被踩/flag/解决率/版本/向量状态）可见；低解决率条目标红浮顶；引用数低于样本下限显示「样本积累中」；点行直达条目效果页签。
```
...
```text
WHEN 知识审核员审核自己提交的条目
THE SYSTEM SHALL 使通过操作不可执行（禁用并说明四眼原则）；须由另一名审核员处理。
```
...
> 原「评测集与放量门禁」节随 F01–F08 与自研引擎废止（golden set 构成/灰度 2 周 20%→50%→100%/放量门禁五项/供应商切换回归等机制 retired，历史见 git commit 671e97a）；评测集机制降级为发布门禁第④查**代理评测**（PRD §4 章头登记）。

- 发布门禁四查（REQ-F09-15 / AC-F09-25）：格式字段完整性 / 内部段落标记 / 英文版本状态 / 代理评测——全过才入同步队列；代理评测=本台向量检索召回验证，结果固定标注「代理指标不等于 Zendesk AI 真实表现」，不得用于对外宣称解决率（PRD §8.3）。
...
- 统一过审铁律（REQ-F09-04 / AC-F09-18）：无人审不生效，任何角色无绕过路径；四眼原则（AC-F09-36）。
...
## 统计（revision 5 · 08-04-2026 架构转向）

- 稳定对象总计：**189**（US×36 + REQ×56 + AC×92 + NFR×5）；其中 **active 54 / retired 135**（retired 保留原 ID 与 EARS 正文为契约史料，永不复用）。
- US：36 注册 = 24 retired（F01–F08×20 含契约层增补 US-F07-01；F09 五区版旧×4）+ 12 active（US-F09-05…16，revision 5 随 PRD §2.4 增补）。
- REQ：56 注册 = 43 retired（F01–F08×37；F09 旧×6：REQ-F09-01/03/05/06/09/10）+ 13 active（修订续用×4：REQ-F09-02/04/07/08（Revision 5）+ 新增×9：REQ-F09-11…19）；active REQ 全部 Anchor=FR-F09、Stage=MVP。
- AC：92 注册 = 65 retired（F01–F08×54；F09 旧×11：AC-F09-01…05/07/08/09/11/12/13）+ 27 active（契约层增补续用×2：AC-F09-06/10 + 新增×25：AC-F09-14…38 ↔ PRD AC-P-01…25）；每条 AC 唯一 Parent，13 个 active REQ 全部由 active AC 覆盖（retired REQ 由 retired AC 覆盖，史料闭合）。
...
- 铁律覆盖核对（对齐 PRD §13.1）：权限矩阵生效=AC-F09-35、统一过审=AC-F09-18、驳回理由必填=AC-F09-24、同步阻断=AC-F09-25/26、版本回滚=AC-F09-19、drift 报警=AC-F09-28、翻译人工校验后才同步=AC-F09-29、四眼原则=AC-F09-36、迁移无绕审=AC-F09-38。
