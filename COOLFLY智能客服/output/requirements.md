...
- Stage: MVP
- Revision: 7（08-05-2026 二轮：正文编辑面改为**双语富文本编辑器**——左中文右英文同屏逐行对齐、富文本行内标记落 `paragraph.html`、「小标题」与「内部段落」两个勾选框取消（分别由 H2/H3 与工具条块级标记按钮承担）；标题单独成卡做中英对照。历史 rev6：**向量状态面板整体删除**，替换为 **AI 摘要面板**——摘要在发布时由 LLM 生成、可人工校正且校正后不被覆盖，只服务本台内部语义查重；效果指标由五卡收敛为四卡（去「向量状态」卡））
- Status: active
- Behavior (EARS):
