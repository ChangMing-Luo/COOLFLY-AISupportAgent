...
- revision: 2
- source_prd: PRD详细版.md（1520 行，commit ec461aa 增补 F09；§13 一致性自检为 ID 权威）
- status: active
...
- **AC 双 ID 制**：PRD §10 的 AC-M·E·S·C·I 系列（共 51 条；revision 2 随 PRD 增 AC-M-13/14/15、AC-E-13/14）保持权威展示 ID 不变（PRD 不改）；本契约为每条 AC 增补机器可解析的契约 ID `AC-Fxx-yy`（xx=所属功能，yy=组内顺序），每个 AC 块内以 `- PRD 对照 ID:` 行保留双向映射，正文与 PRD 交叉引用继续使用 AC-M 系列展示 ID。完整对照表：
...
- **契约层增补对象**（PRD 无对应 ID，标注"契约层增补"）：US-F07-01（F07 架构约束承载故事）；AC-F01-20/21、AC-F02-08、AC-F03-11、AC-F05-02、AC-F06-02/03/04（8 条，行为均提炼自 PRD §5/§7 已写死内容）；AC-F09-06/07（2 条，revision 2 增补，行为提炼自 PRD §5.10 已写死内容）——共 10 条最小 AC，均不新增行为。
...
- US：23（F01×7、F02×3、F03×2、F04×2、F05×1、F06×2、F07×1、F08×2、F09×3；US-F07-01 为契约层增补的架构约束承载故事，PRD §2.4 仍声明 F07 无用户场景故事；US-F09-01/02/03 为 revision 2 随 PRD §2.4 增补）
- REQ：43（F01×11、F02×6、F03×7、F04×2、F05×2、F06×4、F07×2、F08×3、F09×6；全部为本契约以 FR-F0x 为锚增补，Stage 均为 MVP，Status 均为 active；REQ-F09-01…06 为 revision 2 增补，REQ-F05-01/02 语义变更 Revision 1→2）
- AC：61（51 条沿用 PRD §10 原语义、启用契约 ID AC-Fxx-yy 并以 PRD 对照 ID 行保留映射：AC-M×15、AC-E×14、AC-S×11、AC-C×6、AC-I×5，其中 AC-F09-01…05 为 revision 2 随 PRD 增补；另 10 条契约层增补 AC-F01-20/21、AC-F02-08、AC-F03-11、AC-F05-02、AC-F06-02/03/04、AC-F09-06/07；每条唯一 Parent REQ，43 个 REQ 全覆盖）
- NFR：5（NFR-001–005，本契约按模板编号，来源 PRD §5.1/§9.14/§9.9/§4.5/§3.7）
- 占位符：`[PRD 定数 · Gx 后回填]` 与「建议值 · 终稿前确认」全部原样保留（清单权威见 PRD §13.2；revision 2 新增 F09 三项建议值：访问控制/单文件 500 条/标签 ≤20 个）
