...
- revision: 3
- source_prd: PRD详细版.md（1540 行，commit 83754f6 F09 第二轮增强：管理台界面 100% 简体中文 + 知识库效果看板；§13 一致性自检为 ID 权威）
- status: active
...
- **AC 双 ID 制**：PRD §10 的 AC-M·E·S·C·I 系列（共 53 条；revision 2 随 PRD 增 AC-M-13/14/15、AC-E-13/14，revision 3 随 PRD 增 AC-M-16、AC-E-15）保持权威展示 ID 不变（PRD 不改）；本契约为每条 AC 增补机器可解析的契约 ID `AC-Fxx-yy`（xx=所属功能，yy=组内顺序），每个 AC 块内以 `- PRD 对照 ID:` 行保留双向映射，正文与 PRD 交叉引用继续使用 AC-M 系列展示 ID。完整对照表：
...
- **契约层增补对象**（PRD 无对应 ID，标注"契约层增补"）：US-F07-01（F07 架构约束承载故事）；AC-F01-20/21、AC-F02-08、AC-F03-11、AC-F05-02、AC-F06-02/03/04（8 条，行为均提炼自 PRD §5/§7 已写死内容）；AC-F09-06/07（2 条，revision 2 增补，行为提炼自 PRD §5.10 已写死内容）；AC-F09-10（1 条，revision 3 增补，行为提炼自 PRD §5.10「权威源与界面语言」节）——共 11 条最小 AC，均不新增行为。
...
| 飞书知识库 API（120+ 篇**一次性迁移导入** F09 管理台，08-04-2026 拍板改口径，PRD §9.2） | 用户团队 | 用户团队申请 / 内部技术团队接入 | deployment-secret | system | API 权限前置登记；一次性：迁移完成即生命周期结束、不留同步链路，此后管理台=唯一权威源；迁移失败→人工从飞书导出文件（Markdown/CSV）经 F09 批量导入兜底；索引失败分层告警归 §7.4（与飞书无关） | MVP | REQ-F09-02、REQ-F01-02 |
...
## Feature F09 · 知识库管理台（FR-F09 · 应做 🟩 · 引擎线内部 Web 工具；不卡六项及格线验收，同 F05 待遇；08-04-2026 拍板增补，revision 2）

### US-F09-01 · 录入/导入条目并确认 AI 整理建议

- Role: 知识库内容运营（内部工具侧直接角色，PRD §2.1，08-04-2026 增补；拍板 2 名单，2 名）
- Goal: 在管理台录入或导入知识条目（手工表单/飞书一次性迁移/Markdown/CSV 批量导入），并对 AI 切条、问题标签与场景/型号归类建议逐项确认
- Value: 120+ 篇存量与新增内容快速变成可检索的结构化条目
- Stage: MVP（F09 不卡六项及格线验收，同 F05 待遇）
- Status: active

### US-F09-02 · 审核队列处置 AI 起草草稿，无人审不生效

- Role: 知识库内容运营（内部工具侧直接角色，PRD §2.1，08-04-2026 增补）
- Goal: 在审核队列看到 AI 从真实会话聚类起草的条目草稿（含来源会话与频次、标签建议），一键通过发布/驳回/改后发布
- Value: 回流知识安全入库——未经我审核的草稿绝不生效
- Stage: MVP（F09 不卡六项及格线验收，同 F05 待遇）
- Status: active
...
### US-F09-04 · 效果看板定位该优先修哪条（08-04-2026 第二轮拍板增补）

- Role: 知识库内容运营（内部工具侧直接角色，PRD §2.1，08-04-2026 增补）
- Goal: 在管理台效果看板看到每条知识的使用与解决效果（命中/引用/点踩/条目准确率），知道该优先修哪条
- Value: 知识库供给侧效果可见，修订优先级有数据依据（与 F05 回流联动：标黄条目即修订候选）
- Stage: MVP（F09 不卡六项及格线验收，同 F05 待遇）
- Status: active

### REQ-F09-01 · 条目管理（列表/双编辑器/状态流转/修订历史）

- Story: US-F09-01
- Anchor: FR-F09（PRD §5.10 界面元素表/交互逻辑 1）
- Stage: MVP（F09 不卡六项及格线验收，同 F05 待遇）
- Revision: 2
- Status: active
- Behavior (EARS):
...
```text
THE SYSTEM SHALL 提供独立内部 Web 管理台（桌面 1280+ 宽度，界面简体中文，与 F06 看板同体系部署；访问控制「建议值 口令 Basic Auth + 链接不公开 · 终稿前确认」）：条目列表默认按更新时间降序，支持按场景/型号/语言/状态/问题标签组合筛选与搜索，点行进入编辑。
WHEN 运营编辑问答型条目 THE SYSTEM SHALL 提供字段对齐 §4.1 schema 的表单（标题/场景/型号/语言/正文/问题标签），保存生成修订记录。
...
### REQ-F09-02 · 录入与批量导入（一律先进「待整理」队列）
...
```text
WHEN 运营通过手工表单提交条目 THE SYSTEM SHALL 将其送入「待整理」队列或存为草稿，不直接上线。
WHEN 运营批量导入文件（Markdown/CSV；飞书 120+ 篇一次性迁移走同一入口；单文件条目上限「建议值 500 条 · 终稿前确认」）THE SYSTEM SHALL 将导入内容一律先送入「待整理」队列，并逐条报告导入结果。
IF 批量导入部分失败（格式错误/字段缺失）THEN 成功条目照常进「待整理」队列，失败条目逐条报告行号与原因；不整批静默失败、不静默丢行。
IF 飞书一次性迁移失败 THEN 人工从飞书导出文件经批量导入入库（PRD §9.2 兜底），迁移失败不阻塞管理台其余能力。
管理台为知识库唯一权威源：迁移完成后不做与飞书的双向同步（防口径分叉）。
```

### REQ-F09-03 · AI 自动整理（全部建议态）
...
```text
WHEN 内容完成录入/导入 THE SYSTEM SHALL 自动生成 AI 整理建议卡（全部建议态）：①切条建议（长文拆为多条目）②问题标签建议（用户问法变体，多值，上限「建议值 每条目 ≤20 个 · 终稿前确认」）③场景/型号归类建议。
WHEN 运营逐项采纳或修改建议 THE SYSTEM SHALL 按采纳/修改后的内容生效；IF 运营拒绝某建议 THEN 该建议不生效，且不阻塞运营人工直接编辑与发布（AI 建议为建议态、非门槛）。
IF AI 整理/起草服务不可用（LLM 故障）THEN 建议卡隐藏并明示 AI 建议暂不可用；手工录入、编辑、审核、发布全部照常可用（AI 能力为增强不是依赖，与 §7.5 降级原则同源）。
```

### REQ-F09-04 · 会话提炼审核队列与无人审不生效（F05 消化工作面）

- Story: US-F09-02
- Anchor: FR-F09（PRD §5.10 交互逻辑 4/异常场景；§4.4/§5.6 回流管道）
- Stage: MVP（F09 不卡六项及格线验收，同 F05 待遇）
- Revision: 2
- Status: active
- Behavior (EARS):
...
```text
WHEN F05 回流触发源会话（REQ-F05-01 四触发源）累积 THE SYSTEM SHALL 由 AI 聚类并起草条目草稿（含正文、问题标签建议、来源会话链接或脱敏摘要（§4.5 PII 规则）、聚合频次），送入审核队列按频次降序排列，待审数量可见。
WHEN 运营打开单条草稿 THE SYSTEM SHALL 提供三操作：通过发布 / 驳回（须填原因，供起草复盘）/ 改后发布。
无人审不生效（铁律，08-03-2026 拍板确立、08-04-2026 随 F09 落地）：未经审核的草稿与「待整理」条目不进检索库、不被 F01 命中、不出现在任何回答引用中。
IF 审核队列积压 THEN 按频次降序保持可操作，周消化 SLA（Top 10）照常按队列数据判定（§5.6）；积压不影响已上线条目正常服务。
```
...
### REQ-F09-07 · 知识库效果看板（四卡 + 条目效果列表，零新增埋点）

- Story: US-F09-04
- Anchor: FR-F09（PRD §5.10 界面元素表/交互逻辑 7/异常场景，08-04-2026 第二轮拍板增补；指标口径唯一落点 §8.2「知识条目效果」行）
- Stage: MVP（F09 不卡六项及格线验收，同 F05 待遇）
- Revision: 3
- Status: active
- Behavior (EARS):
...
```text
THE SYSTEM SHALL 在管理台第 5 导航区「效果看板」提供只读呈现：四统计卡——场景覆盖（已上线条目按场景分布 + 覆盖场景数，对照 G1 咨询分布场景集）、使用频率（条目被检索命中次数，周趋势）、引用采纳（条目被实际引用进回答的次数，周趋势；与命中分列——命中未被采纳的差值可见）、知识库准确率（整体条目准确率 + 周趋势）——与条目效果列表（逐条目：命中数/引用数/点踩数/「试了没用」数/条目准确率；默认按条目准确率升序、最差条目浮顶，可切换排序列）。
指标口径唯一落点 = PRD §8.2「知识条目效果」行（本 REQ 引用不复制）；全部指标由 §4.2 既有埋点事件聚合产出（零新增埋点），与 F06 业务看板同源、不重复建设（F06 看会话与业务结果，本看板看条目供给侧效果）；数据粒度 = 周。
...
IF 埋点上报延迟 THEN 延迟数据标注「数据回补中」，不静默缺数（与 F06 看板同口径，§5.7）。
```

### REQ-F09-08 · 界面语言（100% 简体中文 + 界面/内容语言分离 + 语言资源加载）

- Story: US-F09-01
- Anchor: FR-F09（PRD §5.10「权威源与界面语言」节，08-04-2026 第二轮拍板强化；管理台全域横切约束，覆盖全部导航区含效果看板）
- Stage: MVP（F09 不卡六项及格线验收，同 F05 待遇）
- Revision: 3
- Status: active
- Behavior (EARS):
...
```text
THE SYSTEM SHALL 使第一版管理台全部界面元素（导航/按钮/标签/提示/占位文案）100% 简体中文，不残留英文装饰性文案（品牌词 "COOLFLY" 允许保留）；中文界面文案唯一落点 = PRD §5.10 文案定稿表。
THE SYSTEM SHALL 保持界面语言与内容语言分离：知识条目内容本身保持英文（服务北美 App 用户，条目带语言字段，§4.1 既有约束）；界面文案全中文不改变条目内容语言，不违反 F07 约束。
...
```text
WHEN F09 管理台内存在未经审核的草稿/待整理条目，App 用户提出该条目覆盖的问题
THE SYSTEM SHALL 保证未审核条目不被检索命中、不出现在任何回答引用中；只有审核通过并过发布门禁的条目进入检索库（无人审不生效铁律，§5.10）。
```

### AC-F09-04 · AI 建议可修改可拒绝且不阻塞人工发布

- PRD 对照 ID: AC-E-13（PRD §10）
- Parent: REQ-F09-03
- Priority: P1
- EARS:
...
THE SYSTEM SHALL 使修改后按运营修改内容生效、拒绝后该建议不生效，且不阻塞运营人工直接编辑与发布（AI 建议为建议态、非门槛，§5.10）。
```
...
THE SYSTEM SHALL 使成功条目照常进入「待整理」队列，失败条目逐条报告行号与原因；不整批静默失败、不静默丢行（行为提炼自 PRD §5.10 异常场景，不新增行为）。
```
...
### AC-F09-08 · 效果看板四卡与条目效果列表可见、口径一致

- PRD 对照 ID: AC-M-16（PRD §10）
- Parent: REQ-F09-07
- Priority: P1
- EARS:

```text
WHEN 知识库内容运营或业务负责人打开 F09 管理台效果看板（第 5 导航区）
THE SYSTEM SHALL 使四卡可见：场景覆盖（已上线条目场景分布+覆盖场景数）/使用频率（检索命中周趋势）/引用采纳（实际引用进回答周趋势，与命中分列——命中未被采纳可见）/知识库准确率（整体+趋势）；条目效果列表可见：逐条目命中数/引用数/点踩数/「试了没用」数/条目准确率，可按准确率升序排列（最差条目浮顶）；全部指标口径与 §8.2「知识条目效果」行一致（§5.10）。
```

### AC-F09-09 · 样本量不足不显示误导性准确率

- PRD 对照 ID: AC-E-15（PRD §10）
- Parent: REQ-F09-07
- Priority: P1
- EARS:

```text
WHEN F09 效果看板中某条目引用数低于样本量下限（「建议值 10 次 · 终稿前确认」）
...
```text
WHEN 知识库内容运营打开第一版管理台任一视图（含效果看板）
THE SYSTEM SHALL 使全部界面元素（导航/按钮/标签/提示/占位文案）以简体中文呈现，无残留英文装饰性文案（品牌词 "COOLFLY" 除外），且知识条目内容语言不因界面语言改变（行为提炼自 PRD §5.10「权威源与界面语言」节，不新增行为）。
```
...
- US：24（F01×7、F02×3、F03×2、F04×2、F05×1、F06×2、F07×1、F08×2、F09×4；US-F07-01 为契约层增补的架构约束承载故事，PRD §2.4 仍声明 F07 无用户场景故事；US-F09-01/02/03 为 revision 2 随 PRD §2.4 增补，US-F09-04 为 revision 3 随 PRD 第二轮增补）
- REQ：45（F01×11、F02×6、F03×7、F04×2、F05×2、F06×4、F07×2、F08×3、F09×8；全部为本契约以 FR-F0x 为锚增补，Stage 均为 MVP，Status 均为 active；REQ-F09-01…06 为 revision 2 增补、REQ-F09-07/08 为 revision 3 增补，REQ-F05-01/02 语义变更 Revision 1→2）
- AC：64（53 条沿用 PRD §10 原语义、启用契约 ID AC-Fxx-yy 并以 PRD 对照 ID 行保留映射：AC-M×16、AC-E×15、AC-S×11、AC-C×6、AC-I×5，其中 AC-F09-01…05 为 revision 2、AC-F09-08/09 为 revision 3 随 PRD 增补；另 11 条契约层增补 AC-F01-20/21、AC-F02-08、AC-F03-11、AC-F05-02、AC-F06-02/03/04、AC-F09-06/07、AC-F09-10；每条唯一 Parent REQ，45 个 REQ 全覆盖）
