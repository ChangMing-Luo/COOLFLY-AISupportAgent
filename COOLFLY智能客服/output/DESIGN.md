...
> 自然绿的冷静安抚系统：为带着挫败进来的中老年用户设计的高可读支持对话界面，浅色单模式，色彩响应交互，来源徽章与 Done / It didn't work 大按钮双选是它的签名。
...
所以这套设计系统的物理感觉是：**一张摊在晨光里的白纸说明书，页边留着大片空白，重点句子用大号字写，每一步只讲一件事。** 背景是极浅的、几乎察觉不到的植物绿灰（#F5F7F5），像清晨透过纱窗的光；消息卡是纯白的纸面（#FFFFFF），浮在背景之上但不投夸张的影子；品牌自然绿（#2E7D4F，待品牌素材校准）只出现在最需要它的地方——那颗告诉你"下一步在这里"的大按钮上。

**用户情绪：从挫败到被接住的踏实感。**

用户是带着故障、带着怒气、甚至带着退货念头进来的（PRD §2.3 旅程 C 是真实差评还原）。因此这套系统的第一情绪使命不是"愉悦"，而是**安抚与可靠**：

- **冷静**：界面上任何时刻都不出现兴奋、催促、闪烁的元素。没有弹跳动画，没有渐变霓虹，没有大面积警示红。哪怕在情绪路径（用户打出 "I want a refund"）里，出口置顶卡也用中性纸面 + 绿色行动按钮呈现——因为此刻刺激性的红色只会给怒气添柴。
...
不是因为浅色安全，而是因为这个产品的核心用户是**中老年、老花镜、户外或窗边强光环境**（喂鸟器装在后院，排障场景经常发生在窗边看着设备的时候）。浅色高对比是这类人群与场景下可读性的最优解：正文 #1C2B22 对背景对比度 13.8:1，远超 WCAG AA。深色模式跟随主 App 能力后置（蓝图 §6 拍板），本期不做，避免双模式维护摊薄单模式的打磨深度。

**为什么是自然绿：**

COOLFLY 卖的是与自然的连接——鸟、后院、清晨。绿色是这个品牌唯一合理的主色（占位色值 #2E7D4F，品牌素材到位后仅校准色值、不动结构）。同时绿色在支持场景有第二重语义红利：它天然是"没问题、往前走"的颜色。当用户满心挫败时,一颗绿色大按钮说的是"这事能解决"。
...
| 色值 | 角色 |
| :--- | :--- |
| #F5F7F5 | 页面背景（植物绿灰，晨光底色） |
| #FFFFFF | 卡片/气泡纸面 |
| #EDF3EE | 温感面板（AI 气泡、徽章底） |
| #1C2B22 | 正文主色（深绿黑） |
| #52655A | 次要文字（灰绿） |
| #2E7D4F | 品牌自然绿 · 主行动色（待品牌素材校准） |
| #23703F | 成功（解决确认） |
| #8A5A00 | 警示（看板超区间等） |
| #B23B2E | 危险（仅小面积状态文字/看板标红，禁大面积） |
...
**色彩哲学：绿色是承诺，中性色是空气，语义色是耳语。**

这套色板的每个值都有一个功能使命和一个情绪使命：

- **#2E7D4F（--accent，待品牌素材校准）**不是随机的绿——它落在"森林绿"与"应用绿"之间：比 Material Green 深（保证白字对比度 5.05:1，可直接做按钮底色），比墨绿亮（不显沉重）。它的情绪使命是"这事能解决"，功能使命是**指出唯一的主行动**。
- **#F5F7F5（--bg）**带 2% 的绿倾向。纯灰 #F5F5F5 是机房的颜色，掺入一丝绿之后它变成了纱窗后的晨光——用户感知不到色相，但感知得到温度。
- **#B23B2E（--danger）**是压暗的陶土红而非信号红。它的使用纪律写死：**只用于小面积状态文字与看板超区间标红；情绪路径界面（出口置顶卡、共情消息）本身禁止任何大面积红色**——对怒气用户，红色是火上浇油。共情场景的视觉语言只允许中性纸面 + 绿色行动按钮。
...
| Token | Hex | 设计理由 |
| :--- | :--- | :--- |
| --bg | #F5F7F5 | 页面底色；与纯白卡片形成 1.06:1 的极轻层差，靠色温而非阴影分层 |
| --surface | #FFFFFF | 消息卡/步骤卡/看板图卡纸面；最高可读性载体 |
| --surface-warm | #EDF3EE | AI 气泡与徽章底的温感绿灰；让 AI 的话比系统色更"有人味" |
| --surface-raised | #FFFFFF | 抬升态（下拉/悬浮层）；浅色模式下与 surface 同值，靠边框与阴影 token 区分 |
| --surface-overlay | #FFFFFF | 模态/底部抽屉（转人工确认面板）最高层 |
| --border | #D7E0DA | 描边绿灰；分隔而不切割 |
| --border-soft | #E6ECE7 | 卡内分隔线（摘要卡字段行间） |

**Data / Accent 色板：**

| Token | Hex | 情感定义 |
| :--- | :--- | :--- |
| --accent | #2E7D4F | 承诺与前进；唯一主行动色（待品牌素材校准） |
| --accent-hover | color-mix 压暗 8% | 手指按上去时颜色沉一分——色彩响应哲学的核心表达 |
| --accent-active | color-mix 压暗 14% | 确认按下；沉两分 |
| --success | #23703F | 解决确认的低语，比 accent 更收敛，避免与主按钮争抢语义 |
| --warn | #8A5A00 | 看板"超预期区间/数据回补中"的克制琥珀 |
| --danger | #B23B2E | 陶土红耳语；小面积状态专用，情绪界面禁大面积 |

**Text 色板：**

| Token | Hex | 可读性分析（对主要背景） |
| :--- | :--- | :--- |
| --fg | #1C2B22 | 对 --bg 13.76:1，对 --surface 14.81:1——老花镜与强光下的安全余量 |
| --fg-2 | #33463B | 对 --surface 10.10:1；卡片副标题层 |
| --muted | #52655A | 对 --bg 5.79:1、--surface 6.24:1；辅助说明仍稳过 AA |
| --meta | #5E7267 | 对 --surface 5.15:1、--bg 4.78:1；时间戳/来源徽章文字，AA 达标 |

对比度验证（WCAG 2.2 AA，输出前已用公式逐对计算）：全部文本/背景配对 ≥ 4.5:1，最低的一对是 --accent 作链接文字对 --bg（4.69:1）；--accent-on（#FFFFFF）对 --accent 按钮底 5.05:1，普通字号按钮达标。
...
浅色系统的 Z 轴靠**纸感**而非发光：层级 0 是绿灰背景（--bg），层级 1 是白色卡片（--surface + --elev-ring 一像素描边），层级 2 是常驻置顶元素（情绪出口卡：--elev-raised 弥散淡影，因为它必须"压"在滚动流之上），层级 3 是转人工底部抽屉（--elev-float + 遮罩）。重要的东西浮上来的方式是"更白、更完整的纸面 + 更大的字"，而不是更粗的边框——边框是切割，纸面是承载；对老花眼，一张干净的白卡远比一圈粗线更易聚焦。**hover 不改变阴影与位置**（交互哲学是色彩响应，§6），阴影只属于静态层级，动态反馈只属于颜色。
...
AI 气泡用温感绿灰纸面（--surface-warm）而非纯白——AI 的话应该比系统提示更有体温；用户气泡用 accent 实底、白字，让用户在流里一眼找到"我说过什么"。这是 accent 在对话页的第二处合法用量（与主按钮并存时，气泡属历史内容不计入当屏 2 处纪律的行动位）。
...
事实性回答末尾的 "Source: Setup Guide"。它是诚实的物证，也是截图可识别的品牌记号：胶囊形、绿灰底、13px 文字加书页圆点。刻意做小——诚实是气质，不是广告。可点（未来接跳转时）但视觉上先是徽章后是链接。
...
- **谦逊**：绿色只出现在该出现的地方，从不刷存在感（accent 每屏 ≤2 处）。
...
**插画/图形指引：** 本期唯一的图形资产是排障配图（路由器示意、指示灯状态等）。风格要求：说明书式线描 + 单色 accent 点缀，白底、圆角 8px 内嵌卡中；禁止照片素材混用（光影噪音）与卡通拟人（对成年挫败用户显得轻佻）。图是增强不是依赖，加载失败时文字必须独立完整（PRD §5.3）。
...
5. **禁止 accent 每屏超过 2 处可见行动位**——绿色的指路价值来自稀缺；到处是绿等于没有路标。
