...
> 完整版见 PRD详细版 §2.4（19 条 US 注册表），下面列必做功能的 P0 用户故事。
...
（F05 应做: US-F05-01 知识库运营每周拿到按频次排序的待补条目清单；F07 无独立 US，仅两条约束）

## 3. 功能需求（精简版）

> 完整 FR 见 PRD详细版 §5。FR 编号为组级 FR-F01…FR-F08，与 F01–F08 一一对应，只增补不重排（R2-裁决 5）。上线线：🟢引擎线=服务端灰度（2 周承诺有效）；🔵发版线=客户端发版（覆盖率随升级率爬坡）。
...
阶段二储备（本期只留接口、零额外开发，详见 PRD详细版 §0.1 后置清单）: ①语音/无屏渠道——接口预留 = §3.7 引擎渠道无关 + channel 字段 + 知识条目渠道中立；②AI 生成型知识库——FR-F05 回流管道升级为 AI 起草+人工审核，接口预留 = §4.1 条目 schema + §4.4 聚合键 + 评测集回归，禁止无人审全自动生效。实现时不得违反上述预留约束。
...
字段: 条目标识（唯一稳定）/ 标题-问题描述 / 场景分类（安装/联网/配对/会员/其他）/ 适用设备型号（可多值，空=通用）/ **语言字段（F07 约束，第一版恒英文，必须存在）** / 正文（第②层引用对象）/ 来源与版本（飞书源文档+更新时间，支撑变更生效 ≤24h）/ 状态（草稿/已上线/待改造）。
...
四触发源: ①检索无命中/拒答 ②消息点踩/会话未解决标记/It didn't work ③转人工会话 ④48h 复问会话 → 待补清单（周度导出，字段: 原始问题/命中文档/会话链接/频次）→ 运营消化 Top 10 → 飞书知识库更新 → 增量重建索引（变更生效 ≤24h）→ 评测集回归（不过则条目回炉）。聚合键: 有命中按条目标识、无命中按规范化问题文本。
...
> 完整 5 维 AC 见 PRD详细版 §10；只定义产品可观察结果，测试方法归 TDD Master。共 46 条: AC-M×12、AC-E×12、AC-S×11、AC-C×6、AC-I×5。

### 7.1 主流程 AC-M-01–12（必过）
...
- **知识检索**: 飞书知识库 API 导出 + 定期增量重建索引，变更生效 ≤24h；权限未就绪时首次全量走人工导出兜底。
...
不依赖 G 数据的「建议值 · 终稿前确认」产品参数（16 项: 超时 8–10 秒、首条流式 ≤3 秒、安抚系数 1.5、滚动窗口近 7 天中位数、看板延迟 ≤24h、日志留存 12 个月、索引上报阈值 48h、输入上限 1,000 字符、摘要补充 500 字符、会话历史 200 条、步骤循环 2 次、转接失败兜底 2 次、超时优化线 5%、双语模板集合 西/法/德、底线态等待窗口 24h、预警载体飞书群机器人）见 PRD详细版 §13.2 单列表；情绪/退款关键词清单 v1 为「建议值 · SOP 岗确认」（§12.7）。
...
output/PRD详细版.md 是完整版（1424 行），本文是浓缩版；产品可观察行为与稳定 ID 以 PRD详细版为准，测试方法由后续 TDD Master 定义。若冲突，停止实现并回到 PRD 修订。
