...
| FR-F09 | 知识库管理台 | 应做🟩（不卡六项及格线验收）🟢（内部 Web，不依赖 App 发版） | 知识库唯一权威源：条目管理 + 录入/导入 + AI 自动整理（建议态）+ 会话提炼审核队列 + 发布门禁 + 效果看板（四卡+条目效果列表，零新增埋点，08-04 第二轮增补）（详见 PRD详细版 §5.10） | 无人审不生效（铁律）；发布须过评测集回归，不过即回炉；界面 100% 简体中文（08-04 第二轮强化：界面/内容语言分离——条目内容仍英文带语言字段；UI 文案从语言资源加载不硬编码）；桌面 1280+；访问控制=Basic Auth+链接不公开（建议值）；批量导入单文件 ≤500 条（建议值）；问题标签每条目 ≤20 个（建议值）；条目准确率标黄线 60%（建议值）；条目准确率样本量下限=引用 10 次（建议值） |
...
> 完整 5 维 AC 见 PRD详细版 §10；只定义产品可观察结果，测试方法归 TDD Master。共 53 条: AC-M×16、AC-E×15、AC-S×11、AC-C×6、AC-I×5（AC-M-13–16 / AC-E-13–15 为 F09 验收，按功能自身验收、不并入六项及格线 MVP 完成判定；AC-M-16 / AC-E-15 为 08-04 第二轮效果看板增补）。

### 7.1 主流程 AC-M-01–16（必过；13–16 为 F09，不卡 MVP 完成判定）
...
| AC-M-16 | 打开管理台效果看板 → 四卡可见（场景覆盖/使用频率/引用采纳（与命中分列）/知识库准确率）+ 条目效果列表（命中/引用/点踩/「试了没用」/条目准确率，可按准确率升序最差浮顶）；口径与 §8.2「知识条目效果」行一致 |
...
不依赖 G 数据的「建议值 · 终稿前确认」产品参数（21 项: 超时 8–10 秒、首条流式 ≤3 秒、安抚系数 1.5、滚动窗口近 7 天中位数、看板延迟 ≤24h、日志留存 12 个月、索引上报阈值 48h、输入上限 1,000 字符、摘要补充 500 字符、会话历史 200 条、步骤循环 2 次、转接失败兜底 2 次、超时优化线 5%、双语模板集合 西/法/德、底线态等待窗口 24h、预警载体飞书群机器人、管理台访问控制 Basic Auth+链接不公开（F09）、批量导入单文件上限 500 条（F09）、问题标签每条目 ≤20 个（F09）、条目准确率标黄线 60%（F09 效果看板·08-04 第二轮）、条目准确率样本量下限 引用 10 次（F09 效果看板·08-04 第二轮））见 PRD详细版 §13.2 单列表；情绪/退款关键词清单 v1 为「建议值 · SOP 岗确认」（§12.7）。
...
5. **产品行为以这些契约为准**: 本文 §7 的 53 条 AC + PRD详细版 §10；用户可见文案唯一落点为 PRD详细版 §5（文案定稿表，照抄勿改写）；测试方法由 TDD Master 生成。
