...
（F05 应做: US-F05-01 知识库运营每周拿到按频次排序的待补条目清单；F09 应做: US-F09-01–04 知识库内容运营在管理台录入/导入（过审后才入总页面）、统一审核人工与 AI 录入条目、确认问题标签、用数据看板定位该优先修哪条，见 PRD详细版 §2.4；F07 无独立 US，仅两条约束）
...
| FR-F09 | 知识库管理台 | 应做🟩（不卡六项及格线验收）🟢（内部 Web，不依赖 App 发版） | 知识库唯一权威源，一级导航五区（08-04 第三轮重构）：①数据看板（知识库数据+客服工作数据双子板，后者=F06 同源）②知识库总页面（书目录式章节树：大章节→小章节→条目）③人工录入（双编辑器+AI 建议卡+批量导入）④AI 录入（每日抓取批次列表）⑤审核队列（人工+AI 统一过审）+ 发布门禁（详见 PRD详细版 §5.10） | 无人审不生效（铁律，第三轮扩展至全部新条目/修订：人工录入也过审核队列）；发布须过评测集回归，不过即回炉；界面 100% 简体中文含示例内容（第三轮扩展；界面/内容语言分离——条目语言录入时选择带语言字段；UI 文案从语言资源加载不硬编码）；章节两级、含条目章节禁删；桌面 1280+；访问控制=Basic Auth+链接不公开（建议值）；批量导入单文件 ≤500 条（建议值）；问题标签每条目 ≤20 个（建议值）；条目准确率标黄线 60%（建议值）；条目准确率样本量下限=引用 10 次（建议值）；AI 录入抓取节奏=每日（建议值·08-04 第三轮） |
...
1. **一级导航五区（08-04 第三轮重构）**: 数据看板（知识库数据+客服工作数据双子板）、知识库总页面（章节树两级+搜索筛选+双编辑器/状态流转/修订历史）、人工录入（手工表单 + Markdown/CSV 批量导入 + AI 建议卡）、AI 录入（每日抓取批次列表）、审核队列（人工+AI 统一过审，F05 消化工作面）。
2. **无人审不生效（铁律，第三轮扩展）**: 人工录入、批量导入与 AI 起草条目统一进审核队列，未经运营审核不进知识库总页面、不进检索库、不被 F01 命中（AC-M-15/AC-M-18）。
3. **发布门禁**: 任何条目转入「已上线」触发评测集回归，结果留痕；回归不过→条目回炉、失败用例可见（AC-E-14）；通过→索引更新，变更生效 ≤24h。
4. **问题标签=检索信号**: 人工确认后的问题标签扩充 F01 召回+过滤；命中率仍以评测集验收，§8 目标值不变、不新增指标（AC-M-13）；标签管理不占一级导航（第三轮拍板，确认收进录入/审核流程与总页面筛选）。
5. **飞书一次性迁移**: 120+ 篇存量经批量导入进审核队列为初始数据（原「待整理」队列已并入审核队列），此后管理台为唯一权威源，不做双向同步；迁移失败走人工导出文件批量导入兜底。
...
> 完整 5 维 AC 见 PRD详细版 §10；只定义产品可观察结果，测试方法归 TDD Master。共 55 条: AC-M×18、AC-E×15、AC-S×11、AC-C×6、AC-I×5（AC-M-13–18 / AC-E-13–15 为 F09 验收，按功能自身验收、不并入六项及格线 MVP 完成判定；AC-M-16 / AC-E-15 为 08-04 第二轮效果看板增补，AC-M-17 / AC-M-18 为 08-04 第三轮章节树/统一过审增补）。

### 7.1 主流程 AC-M-01–18（必过；13–18 为 F09，不卡 MVP 完成判定）
...
| AC-M-15 | 未经审核的条目（含人工录入与批量导入，第三轮统一过审扩展）→ 不被检索命中、不出现在任何回答引用中（无人审不生效铁律） |
| AC-M-16 | 打开管理台数据看板·知识库数据子看板（第三轮改组归入①区，内容不变）→ 四卡可见（场景覆盖/使用频率/引用采纳（与命中分列）/知识库准确率）+ 条目效果列表（命中/引用/点踩/「试了没用」/条目准确率，可按准确率升序最差浮顶）；口径与 §8.2「知识条目效果」行一致 |
| AC-M-17 | 打开知识库总页面 → 条目按大章节→小章节两级书目录式层级组织可导航（章节树折叠展开、条目挂小章节下、点条目进编辑器）；章节可新建/重命名/调整层级；搜索筛选层级下保留可用（08-04 第三轮增补） |
| AC-M-18 | 人工录入提交后、审核前 → 条目仅出现在审核队列（来源标注「人工录入」），不出现在知识库总页面、不进检索库；过审+发布门禁后才转入总页面对应章节（08-04 第三轮增补） |

### 7.2 异常分支 AC-E-01–15（必过；13–15 为 F09，不卡 MVP 完成判定）
...
| AC-E-13 | 运营对 AI 切条/标签/场景型号/所属章节归类建议逐项修改或拒绝 → 按修改内容生效/拒绝不生效，且不阻塞提交进审核队列（建议态非门槛；条目仍须过审后入库——08-04 第三轮随统一过审语义修订） |
| AC-E-14 | 发布时评测集回归失败 → 条目不生效、状态回炉，失败原因与用例可见，修订后可重新发布 |
| AC-E-15 | 条目引用数低于样本量下限（建议值 10 次）→ 不显示准确率数值、显示「样本积累中」；其余指标照常显示（不显示误导性准确率） |
...
不依赖 G 数据的「建议值 · 终稿前确认」产品参数（22 项: 超时 8–10 秒、首条流式 ≤3 秒、安抚系数 1.5、滚动窗口近 7 天中位数、看板延迟 ≤24h、日志留存 12 个月、索引上报阈值 48h、输入上限 1,000 字符、摘要补充 500 字符、会话历史 200 条、步骤循环 2 次、转接失败兜底 2 次、超时优化线 5%、双语模板集合 西/法/德、底线态等待窗口 24h、预警载体飞书群机器人、管理台访问控制 Basic Auth+链接不公开（F09）、批量导入单文件上限 500 条（F09）、问题标签每条目 ≤20 个（F09）、条目准确率标黄线 60%（F09 数据看板·08-04 第二轮）、条目准确率样本量下限 引用 10 次（F09 数据看板·08-04 第二轮）、AI 录入抓取节奏 每日（F09·08-04 第三轮））见 PRD详细版 §13.2 单列表；情绪/退款关键词清单 v1 为「建议值 · SOP 岗确认」（§12.7）。
...
5. **产品行为以这些契约为准**: 本文 §7 的 55 条 AC + PRD详细版 §10；用户可见文案唯一落点为 PRD详细版 §5（文案定稿表，照抄勿改写）；测试方法由 TDD Master 生成。
